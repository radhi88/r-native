...
input bool    InpUseATRMinSL       = true;         // reject SL closer than InpATRMinSLMult * ATR
input double  InpATRMinSLMult      = 1.5;          // minimum SL = ATR * this multiplier
...
input bool    InpUseEMATrendFilter = true;          // require EMA50>EMA200 for BUY / EMA50<EMA200 for SELL
...
int           h_filt_ema50  = INVALID_HANDLE;   // trend filter EMA50
int           h_filt_ema200 = INVALID_HANDLE;   // trend filter EMA200
...
bool PlaceBrainOrder(string side, double entry, double sl, double tp, double lot, string reason) {
   ...
   if (InpUseATRMinSL && sl > 0 && h_atr != INVALID_HANDLE) {
      double atr_val = LastBuf(h_atr);
      if (atr_val > 0) {
         double atr_pt  = atr_val / Sym.Point();                  // ATR in points (= ATR * 100 for gold)
         double sl_dist = MathAbs(entry - sl) / Sym.Point();      // SL distance in points
         if (sl_dist < atr_pt * InpATRMinSLMult) {
            g_last_reason = StringFormat("ATR SL reject: %.0fpt < %.1fx ATR(%.0fpt)",
                                          sl_dist, InpATRMinSLMult, atr_pt);
            return false;
         }
      }
   }
   ...
   // EMA trend filter — blocks counter-trend orders
   if (InpUseEMATrendFilter && h_filt_ema50 != INVALID_HANDLE && h_filt_ema200 != INVALID_HANDLE) {
      double close0    = iClose(InpSymbol, _Period, 0);
      double ema50val  = LastBuf(h_filt_ema50);
      double ema200val = LastBuf(h_filt_ema200);
      bool   filterOk  = false;
      if (side == "BUY")  filterOk = (close0 > ema50val && ema50val > ema200val);
      if (side == "SELL") filterOk = (close0 < ema50val && ema50val < ema200val);
      if (!filterOk) { g_last_reason = "EMA trend filter blocked"; return false; }
   }
...
int InitIndicators() {
   ...
   // Trend filter handles — always initialised regardless of InpShowEMA
   h_filt_ema50  = iMA(InpSymbol, _Period, 50,  0, MODE_EMA, PRICE_CLOSE);
   h_filt_ema200 = iMA(InpSymbol, _Period, 200, 0, MODE_EMA, PRICE_CLOSE);
...
void OnDeinit(const int reason) {
   ...
   if (h_filt_ema50  != INVALID_HANDLE) IndicatorRelease(h_filt_ema50);
   if (h_filt_ema200 != INVALID_HANDLE) IndicatorRelease(h_filt_ema200);
