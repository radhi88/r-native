...
from mt5_ai.agents.agent_runner import AgentRunner
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_GET(self):
                ...
                self._send_json(
                    {
                        ...
                        "safety": {
                            ...
                            "live_trading_reason": "live_trading_permanently_disabled",
                            "allowed_execution_modes": ["paper", "demo"],
                            "mt5_source_means": "data_feed_only",
                        },
            ...
            if parsed.path == "/api/mt5/stream":
                ...
                return
            if parsed.path == "/api/market/live":
                self._send_json(app.mind.market_snapshot(app.assistant))
                return
        ...
        def do_POST(self):
            ...
            try:
                ...
                if parsed.path == "/api/autopaper/mode":
                    mode = str(payload.get("mode", "paper")).strip().lower().replace("demo_mt5", "demo")
                    if mode == "live":
                        self._send_json({
                            "ok": False,
                            "error": "live execution is permanently disabled",
                            "mode": app.assistant.state.mode,
                            "allowed_modes": ["paper", "demo"],
                        }, status=400)
                        return
                    if mode not in {"paper", "demo"}:
                        self._send_json({"ok": False, "error": "mode must be paper or demo"}, status=400)
                        return
                    auto_status = app.auto_paper.set_execution_mode(mode)
                    assistant_msg = app.assistant.set_mode(mode)
                    self._send_json({
                        "ok": True,
                        "message": assistant_msg,
                        "assistant_mode": app.assistant.state.mode,
                        "status": auto_status,
                    })
                    return
                ...
                if parsed.path == "/api/mode":
                    mode = str(payload.get("mode", "paper")).strip().lower().replace("demo_mt5", "demo")
                    if mode == "live":
                        msg = app.assistant.set_mode("live")
                        self._send_json({
                            "ok": False,
                            "message": msg,
                            "mode": app.assistant.state.mode,
                        ...
                        return
                    msg = app.assistant.set_mode(mode)
                    ok = app.assistant.state.mode == mode
                    self._send_json({
                        "ok": ok,
                        "message": msg,
                        "mode": app.assistant.state.mode,
                        "status": app.assistant.status(),
                    }, status=200 if ok else 400)
                    return
                if parsed.path == "/api/obey":
                    symbol = payload.get("symbol") or "XAUUSDm"
                    mode = str(payload.get("mode", "demo")).lower()
                    if mode == "live":
                        mode = "demo"
                    data = app.assistant.obey_aggressive_scalping(symbol=symbol, mode=mode)
                    auto_status = app.auto_paper.set_execution_mode(app.assistant.state.mode)
                    app.auto_paper.start()
                    app.events.publish("obey", data)
                    self._send_json({"ok": True, "data": data, "auto_paper": auto_status})
                    return
