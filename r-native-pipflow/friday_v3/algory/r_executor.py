...
def loop(mode: str, interval: int = 30):
    ...
    # Balance-gate hardening: require N consecutive *valid* sub-threshold reads
    # before exiting. A balance of 0.0/None is treated as connection-loss
    # (terminal reconnect), not a real "out of money" condition — so a single
    # garbage tick can no longer permanently kill the live trader.
    low_balance_strikes = 0
    LOW_BALANCE_STRIKES_REQUIRED = 3   # ~3 reads (90s @ 30s interval) of genuine low balance
    ...
    while True:
        try:
            ...
            # Balance gate (hardened against transient/garbage MT5 reads)
            info = mt5.account_info()
            # None or balance<=0 / equity<=0 => treat as connection-loss, NOT a stop.
            # mt5.account_info() can briefly return 0.0 during a terminal reconnect.
            if info is None or info.balance <= 0.0 or info.equity <= 0.0:
                low_balance_strikes = 0  # do not count bad-data reads toward exit
                _log(state, "⚠ account_info unavailable / balance|equity<=0 "
                            "(treating as connection loss — attempting reconnect)")
                try:
                    mt5.initialize()  # best-effort re-establish terminal link
                except Exception as _re:
                    _log(state, f"⚠ reconnect attempt err: {_re}")
                _save_state(state)
                time.sleep(interval); continue
            # Genuine sub-threshold balance: require N consecutive confirmations.
            if info.balance < R_MIN_BALANCE_USD:
                low_balance_strikes += 1
                _log(state, f"💸 balance ${info.balance:.2f} < ${R_MIN_BALANCE_USD} "
                            f"(strike {low_balance_strikes}/{LOW_BALANCE_STRIKES_REQUIRED})")
                if low_balance_strikes >= LOW_BALANCE_STRIKES_REQUIRED:
                    _log(state, f"💸 balance ${info.balance:.2f} < ${R_MIN_BALANCE_USD} "
                                f"confirmed over {LOW_BALANCE_STRIKES_REQUIRED} reads — exiting")
                    state["armed"] = False
                    _save_state(state); return
                _save_state(state)
                time.sleep(interval); continue
            # Healthy balance read — reset the strike counter.
            low_balance_strikes = 0
