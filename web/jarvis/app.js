...
function appendEdexLog(text, cls = "status") {
  const logEl = document.getElementById("edexLog");
  if (!logEl) return;
  const div = document.createElement("div");
  div.className = `edex-log-line ${cls}`;
...
async function refreshEdex() {
  ...
  try {
    ...
    appendEdexLog("MT5 data feed unavailable: network/API error", "warn");
    return;
  }
  if (data.error) {
    appendEdexLog(`MT5 data feed unavailable: ${data.error}`, "warn");
    return;
  }
  ...
  document.getElementById("edexSrc").textContent    = data.symbol ? `${data.symbol} DATA` : "MT5 DATA";
...
async function refreshMt5Log() {
  ...
  try {
    ...
    if (data.error || !data.lines) {
      appendEdexLog(`MT5 terminal log unavailable: ${data.error || "no lines"}`, "warn");
      return;
    }
...
function handleMultiScan(payload) {
  ...
  buys.forEach(r => buyEl.appendChild(renderScanRow(r,  "buy")));
  sells.forEach(r => sellEl.appendChild(renderScanRow(r, "sell")));
  if (!buys.length) {
    const div = document.createElement("div");
    div.className = "scanner-empty";
    div.textContent = "No BUY setup passed filters yet. Waiting for probability, SMC, spread, and trend conditions.";
    buyEl.appendChild(div);
  }
  if (!sells.length) {
    const div = document.createElement("div");
    div.className = "scanner-empty";
    div.textContent = "No SELL setup passed filters yet. Waiting for probability, SMC, spread, and trend conditions.";
...
const liveMt5Btn = document.getElementById("liveMt5Btn");
...
async function refreshStatus() {
  try {
    ...
    const sourceLabel = assistant.source === "mt5" ? "mt5 data" : (assistant.source || "source");
    const liveLabel = data.safety?.live_trading_enabled ? "live enabled" : "live blocked";
    statusLine.textContent = `${assistant.mode || "paper"} | ${assistant.profile || "profile"} | ${sourceLabel} | ${spreadMode} | ${liveLabel} | mic ${voice.running ? "on" : "off"} | auto ${autoPaper.running ? execMode : "off"}`;
    ...
    if (autoLiveBtn) {
      autoLiveBtn.textContent = "LIVE BLOCKED";
      autoLiveBtn.classList.add("active");
      autoLiveBtn.title = "Live auto execution is blocked in backend; use DEMO MT5 or PAPER.";
    }
    if (liveMt5Btn) {
      liveMt5Btn.textContent = assistant.source === "mt5" ? "MT5 DATA ON" : "MT5 DATA";
      liveMt5Btn.classList.toggle("active", assistant.source === "mt5");
      liveMt5Btn.title = "MT5 market/account data feed only. This does not enable live order execution.";
    }
...
function wireEvents() {
  ...
  events.addEventListener("agent_data_source", (event) => {
    ...
    if (src === "mt5_live") {
      document.getElementById("edexSrc").textContent = "MT5 DATA";
      if (!edexVisible) {
        edexBtn.textContent = "eDEX DATA";
...
demoMt5Btn.addEventListener("click", async () => {
  ...
  if (current === "demo") {
    await api("/api/autopaper/mode", { mode: "paper" });
    await api("/api/mode", { mode: "paper" });
    addMessage("friday", "تم التحويل إلى PAPER.");
  } else {
    await api("/api/obey", { symbol: "XAUUSDm", mode: "demo" });
    addMessage("friday", "تم: DEMO MT5 + XAUUSDm + Scalping + تجاهل السبريد. LIVE AUTO يبقى محجوب.");
  }
...
autoLiveBtn.addEventListener("click", () => {
  addMessage("friday", "LIVE AUTO is blocked by the backend. Available modes: PAPER and DEMO MT5 only.");
});

liveMt5Btn.addEventListener("click", async () => {
  try {
    const data = await api("/api/source", { source: "mt5" });
    addMessage("friday", `${data.message}. MT5 data feed enabled; live execution remains blocked.`);
    if (!edexVisible) edexBtn.click();
  } catch (error) {
    addMessage("friday", `MT5 data feed error: ${error.message}`);
  } finally {
    refreshStatus();
  }
});

tradeBtn.addEventListener("click", () => sendText("فرايدي نفذي ديمو MT5 إذا الصفقة مناسبة"));
...
// ── Auto-start: open eDEX + switch to DEMO MT5 on page load; live execution stays blocked ───────────────────
