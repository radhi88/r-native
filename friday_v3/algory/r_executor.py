...
# +$904 (PF 3.36). Honored here as a NEW-ENTRY veto: open R positions are still
# managed/trailed; only fresh risk pauses in the window. Fail-OPEN — if the
# reflection module/file is missing or unreadable it NEVER blocks; it can only
# ever ADD a restriction, never place or size a trade.
...
# Reconciliation: if a closed ticket's closing deal never appears in history,
# give up after this many seconds so the 0.5s manage loop stops spamming the
# log forever (caught live 2026-06-27: ticket 1358973251 logged ~250×/min).
R_RECONCILE_GIVEUP_S   = 120.0
...
def _save_state(state: dict):
    # Atomic + non-raising. State persistence is NON-critical: real positions
    # live on the broker, not in this file. A transient FS error (concurrent
    # read by the brain server / dashboard, AV scan, Windows Errno 22) must
    # NEVER crash the trading daemon — write to a temp file then atomically
    # replace, and swallow any failure (best-effort).
    state["ts"] = _now_iso()
    tmp = None
    try:
        ...
        tmp.write_text(json.dumps(state, ensure_ascii=False, indent=2),
                       encoding="utf-8")
        # os.replace transiently fails on Windows (WinError 5 Access denied /
        # WinError 32 sharing violation) when the brain server or dashboard has
        # STATE_FILE open for a read at the same instant. The lock clears in a
        # few ms, so retry with a short backoff before giving up — a single
        # attempt was silently freezing the state file for hours.
        last_err = None
        ...
        try:
            # never leave an orphan .tmp behind
            if tmp is not None and tmp.exists():
                tmp.unlink()
        except Exception:
            pass
        try:
            print(f"⚠ _save_state non-fatal write error: {e}", file=sys.stderr)
        except Exception:
            pass
...
def reconcile_closed_positions(state: dict, mode: str):
    ...
    # Accept ANY non-opening deal (OUT=1, INOUT=2, OUT_BY=3) as the close —
    # partial closes / broker close-by would otherwise never match entry==1
    # and the ticket would stay "unreconciled" forever.
    matching = [d for d in deals if d.position_id == last_ticket and int(d.entry) != 0]
    if not matching:
        # Closing deal not visible yet. Track how long we've waited on THIS
        # ticket; give up after R_RECONCILE_GIVEUP_S so the 0.5s manage loop
        # doesn't spam "closed but no closing deal yet" every tick forever.
        _now = datetime.now().timestamp()
        _unrec = state.get("_unreconciled") or {}
        if _unrec.get("ticket") != last_ticket:
            state["_unreconciled"] = {"ticket": last_ticket, "since": _now}
            _log(state, f"   ticket {last_ticket} closed but no closing deal yet (waiting)")
        elif _now - _unrec.get("since", _now) > R_RECONCILE_GIVEUP_S:
            _log(state, f"   ⏭ ticket {last_ticket} unreconciled after "
                        f"{int(_now - _unrec['since'])}s — giving up (P/L not booked)")
            state["last_ticket"] = None
            state["_unreconciled"] = None
        # else: still inside the grace window — stay silent (no spam)
        return

    state["_unreconciled"] = None   # found it — clear any stale tracker
    closing = matching[-1]
    ...
    try:
        ...
        try:
            ...
            sym = getattr(closing, "symbol", R_SYMBOL)
            # Entry hour (UTC) of this position — broker server time ≈ UTC on
            # this account, so the position open epoch maps straight to the hour.
            try:
                from datetime import datetime as _dt, timezone as _tz
                _entry_hour = _dt.fromtimestamp(int(getattr(closing, "time", 0)), tz=_tz.utc).hour
            except Exception:
                _entry_hour = None
            symrec = record_symbol_trade(sym, profit, hour_utc=_entry_hour)
...
def _try_enter_one_symbol(state: dict, mode: str, force_sym: str):
    ...
    # REFLECTION DISCIPLINE GATE — night-trade block etc. from reflection/
    # strategy.json. NEW-entry veto only (open R positions are still managed).
    # Placed early so the blocked window also skips the gate HTTP call. Fail-open.
    _refl_block = _reflection_entry_block(best_sym)
    if _refl_block:
        state["last_action"] = f"discipline block: {_refl_block}"
        _log(state, f"  🌙 reflection night-block {best_sym}: {_refl_block}")
        _write_brain_json({"verdict": "WAIT", "reason_ar": f"discipline: {_refl_block}"},
...
def loop(mode: str, interval: int = 30, manage_interval: float = 0.5):
    """Three-speed loop for split-second reaction.

    * FAST tick  (``manage_interval``, default 0.5s) — reconcile + trailing-SL +
      freeze checks EVERY cycle, so open positions react to price within a
      fraction of a second instead of the old 30s.
    * SLOW tick  (``interval``, default 30s) — the heavy entry gate-scan.
      Faster *entries* are not an edge (see explosion_ride study), but faster
      *risk management* is pure upside (tighter trails, quicker exits).
    * SAVE tick  (~2s) — the state JSON is throttled so the sub-second loop
      doesn't thrash disk; a save is forced whenever something meaningful
      happens (entry scan, freeze transition, position closed).
    """
    ...
    print(f"Cadence: manage={manage_interval}s  scan={interval}s")
    print()
    ...
    _last_scan = 0.0   # monotonic ts of last entry gate-scan
    _last_save = 0.0   # monotonic ts of last state write
    _SAVE_EVERY = 2.0  # throttle state writes to >= this many seconds
    _prev_open = -1    # open-position count last tick (detect closes → force save)
    ...
    while True:
        try:
            ...
            # --- FAST TICK: react to price every manage_interval ---
            ...
            # Detect a position closing this tick → persist immediately
            try:
                _open_now = len(_r_positions())
            except Exception:
                _open_now = _prev_open
            if _prev_open != -1 and _open_now != _prev_open:
                force_save = True
            _prev_open = _open_now

            # Freeze checks (also resets day counters)
            frozen = check_freeze(state)

            # --- SLOW TICK: entry gate-scan only every `interval` seconds ---
            now = time.monotonic()
            if frozen:
                if state.get("last_action") != "FROZEN":
                    force_save = True
                state["last_action"] = "FROZEN"
            elif (now - _last_scan) >= interval:
                _last_scan = now
                try_enter_trade(state, mode)
                force_save = True

            # Throttled / forced state write
            if force_save or (now - _last_save) >= _SAVE_EVERY:
                _save_state(state)
                _last_save = now
...
if __name__ == "__main__":
    ...
    ap.add_argument("--interval", type=float, default=30,
                    help="entry gate-scan cadence in seconds (heavy; floats OK, e.g. 0.1)")
    ap.add_argument("--manage-interval", type=float, default=0.5,
                    help="fast position-management tick in seconds (sub-second OK)")
    ...
    # ── Single-instance guard (LIVE only) ────────────────────────────────
    # Two LIVE executors on magic 20260605 double-submit orders, run
    # competing SL/trail managers, and collide writing the same .tmp state
    # files. Bind a fixed loopback port keyed to the executor as a
    # process-wide mutex; a second LIVE executor finds it taken and aborts.
    # PAPER mode is left unguarded (harmless, useful for parallel testing).
    # The socket auto-releases on process death — no stale-lock risk.
    _live_lock = None
    if mode == "LIVE":
        import socket as _socket
        try:
            _live_lock = _socket.socket(_socket.AF_INET, _socket.SOCK_STREAM)
            _live_lock.bind(("127.0.0.1", 57322))  # dedicated r_executor LIVE port
            _live_lock.listen(1)
        except OSError:
            print("[r_executor] another LIVE executor (magic 20260605) already "
                  "holds the single-instance lock — this duplicate will exit.",
                  flush=True)
            raise SystemExit(0)
        except Exception:
            _live_lock = None  # fail open: never block trading on an odd error

    loop(mode, args.interval, args.manage_interval)
