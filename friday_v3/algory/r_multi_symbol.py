...
# Per-hour-of-day quality (TIGHTEN-ONLY: an hour can only ever VETO, never loosen).
# We only call an hour "BAD" for a symbol once it has clearly, repeatedly lost there.
HOUR_MIN_TRADES   = 4      # need >= this many closed trades in that hour to judge it
HOUR_BAD_WR       = 30.0   # win-rate (%) below this = bad hour ...
HOUR_BAD_NET_USD  = -1.0   # ... AND net P/L below this (both must hold)
...
def record_symbol_trade(symbol: str, profit: float, hour_utc: Optional[int] = None):
    """After a closed trade, update the symbol book + maybe blacklist/whitelist.

    If ``hour_utc`` (0-23, the trade's ENTRY hour in UTC) is given, also tracks
    a per-hour-of-day bucket so the gate can later veto symbols at hours where
    they have repeatedly lost (see ``hour_quality``).
    """
    book = _load_book()
    ...
    # ── Per-hour-of-day bucket + summary win-rate map ──
    try:
        h = int(hour_utc) if hour_utc is not None else None
    except (TypeError, ValueError):
        h = None
    if h is not None and 0 <= h <= 23:
        hours = rec.setdefault("hours", {})
        hb = hours.setdefault(str(h), {"trades": 0, "wins": 0, "net_pl": 0.0})
        hb["trades"] += 1
        hb["net_pl"] = round(hb.get("net_pl", 0) + float(profit), 2)
        if profit > 0:
            hb["wins"] += 1
        # Rebuild the compact hours_pass_rate summary ({hour: win_rate%}) the
        # dashboard/gate read without parsing the full buckets.
        rec["hours_pass_rate"] = {
            hh: round(b["wins"] / b["trades"] * 100, 1)
            for hh, b in hours.items() if b.get("trades", 0) > 0
        }
...
def hour_quality(symbol: str, hour_utc: int) -> dict:
    """Verdict for trading ``symbol`` at ``hour_utc`` (0-23, UTC entry hour).

    TIGHTEN-ONLY: only ever returns BAD (a veto) once the symbol has clearly,
    repeatedly lost at that hour. Sparse / mixed / winning history → NEUTRAL.
    Fail-open: any error or missing data → NEUTRAL (never blocks).

    Returns: {verdict: GOOD|NEUTRAL|BAD, trades, win_rate, net_pl, reason}.
    """
    out = {"verdict": "NEUTRAL", "trades": 0, "win_rate": 0.0, "net_pl": 0.0,
           "reason": "no per-hour data"}
    try:
        h = int(hour_utc)
        if not (0 <= h <= 23):
            return out
        rec = _load_book().get(symbol, {})
        hb = (rec.get("hours") or {}).get(str(h))
        if not hb:
            return out
        n  = int(hb.get("trades", 0))
        wr = round(hb.get("wins", 0) / n * 100, 1) if n else 0.0
        net = round(float(hb.get("net_pl", 0)), 2)
        out.update(trades=n, win_rate=wr, net_pl=net)
        if n < HOUR_MIN_TRADES:
            out["reason"] = f"only {n}/{HOUR_MIN_TRADES} trades at {h:02d}h — too few to judge"
            return out
        if wr < HOUR_BAD_WR and net < HOUR_BAD_NET_USD:
            out["verdict"] = "BAD"
            out["reason"] = f"{symbol} at {h:02d}h: WR {wr}% / net ${net} over {n} trades"
        elif wr >= WHITELIST_WR_ABOVE and net > 0:
            out["verdict"] = "GOOD"
            out["reason"] = f"{symbol} at {h:02d}h: WR {wr}% / net ${net} over {n} trades"
        else:
            out["reason"] = f"{symbol} at {h:02d}h: WR {wr}% / net ${net} (no clear edge)"
    except Exception:
        pass
    return out
