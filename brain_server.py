...
def api_brain():
    ...
    friday_brain.py (magic 99791) is legacy/display and can die silently; its state
    file then freezes but still says live:true. Inject honest liveness so the dashboard
    is not misled (see project_watchdog_silent_death_2026-06-18)."""
    state = _json(BRAIN_V2, {"status": "brain v2 not running"})
    if isinstance(state, dict):
        ...
        state["reported_live"] = reported
        state["live"] = reported and not stale   # honest: stale brain is NOT live
        state["stale"] = stale
        state["state_age_seconds"] = age
        state["brain_status"] = (
            ("OFFLINE_STALE" if age is not None else "OFFLINE_NO_STATE") if stale
            else ("LIVE" if reported else "PAPER")
        )
...
def api_r_force_trade():
    ...
    try:
        ...
        # 🎛️ حوكمة المايسترو: إن أُقيل هذا المحرّك (نزّاف كارثيّ صافي-التكلفة) ⇒ لا فتح (إدارة فقط). يجعل
        # brain_server محكوماً مثل بقيّة الأسطول (كان نزّافاً غير محكوم: −$96/12يوم، توقّع −$1.71/صفقة).
        try:
            from engine_gov import is_paused
            if is_paused(20260605):
                return jsonify({"ok": False, "vetoed": True, "reason": "maestro: مُقال (نزّاف كارثيّ) — لا فتح"})
        except Exception:
            pass  # fail-open
...
@app.route("/api/r/trade_why/<ticket>")
def api_r_trade_why(ticket):
    """WHY a specific trade opened — full reasoning snapshot from the
    per-genome decision_log (indicators, gate verdict, archetype, signals/
    biases/filters). Powers the '?' button on the recent-trades table.
    Scans every decision-log file for the OPEN (and matching CLOSE) record."""
    dlog_dir = Path(r"C:\Users\Radhi\MT5\data\r_native\decision_log")
    try:
        tk = int(ticket)
    except Exception:
        return jsonify({"ok": False, "found": False, "error": "bad ticket"})
    open_rec = None
    close_rec = None
    try:
        if dlog_dir.exists():
            for fp in sorted(dlog_dir.glob("*.jsonl")):
                for ln in fp.read_text(encoding="utf-8", errors="ignore").splitlines():
                    ...
                    try:
                        ...
                        continue
                    if row.get("ticket") != tk:
                        continue
                    if row.get("kind") == "OPEN" and open_rec is None:
                        open_rec = row
...
_IND_MATRIX_CACHE = {}      # symbol -> {"ts":, "data":}
_IND_MATRIX_TTL_S = 30


@app.route("/api/r/indicator_matrix")
def api_r_indicator_matrix():
    """Full 21-indicator × multi-timeframe confluence matrix for a symbol.

    Ports the user's FPU-MAX Pine suite to Python so R reasons on breadth of
    agreement across M5/M15/H1/H4/D1/W1 instead of ATR+RSI alone. Bars are
    fetched from brain_server's shared, already-alive mt5 handle (NO new
    connection — respects the IPC-flood lesson)."""
    ...
    try:
        from friday_v3.algory import indicator_matrix as _im
    except Exception as e:
        return jsonify({"ok": False, "error": f"module load: {e}"})

    now = time.time()
    cached = _IND_MATRIX_CACHE.get(sym)
    if cached and (now - cached["ts"]) < _IND_MATRIX_TTL_S:
    ...
    if not bars_by_tf:
        return jsonify({"ok": False, "error": "no bars", "symbol": sym})

    try:
        matrix = _im.build_matrix(bars_by_tf)
    except Exception as e:
        return jsonify({"ok": False, "error": f"compute: {e}", "symbol": sym})

    data = {"ok": True, "symbol": sym, "matrix": matrix, "ts": now}
    _IND_MATRIX_CACHE[sym] = {"ts": now, "data": data}
    return jsonify(data)
...
@app.route("/api/r/candles")
def api_r_candles():
    """OHLC candles for Lightweight-Charts on /r/, plus R's live entry/SL/TP
    on that symbol so the UI can mark exactly what R sees.

    Query: symbol (optional — auto-picks R's open position or best_symbol_now),
           tf (default M5), n (bars, 20..300, default 150).
    Returns {ok, symbol, tf, candles:[{time,open,high,low,close}],
             positions:[{ticket,type,entry,sl,tp,profit,time_open}]}.
    Cached per (symbol,tf,n) for ~12s to spare MT5.
    """
    if not HAS_MT5:
        return jsonify({"ok": False, "error": "no mt5"})
    sym = request.args.get("symbol", "").strip()
    tf_name = request.args.get("tf", "M5").upper()
    try:
        n = max(20, min(300, int(request.args.get("n", "150"))))
    except ValueError:
        n = 150

    # Auto-pick symbol: prefer an open R position, else executor best_symbol_now.
    if not sym:
        try:
            _ensure_mt5_alive()
    ...
    if not sym:
        try:
            st = json.loads(Path(r"C:\Users\Radhi\MT5\friday_v3\data\r_executor_state.json")
                            .read_text(encoding="utf-8"))
            sym = (st.get("best_symbol_now") or "").strip()
        except Exception:
            pass
    if not sym:
        sym = "XAUUSDm"
    ...
    if cached and (now - cached["ts"]) < _CANDLE_TTL_S:
        # Positions move faster than candles — refresh only the position overlay.
        data = dict(cached["data"])
        data["positions"] = _r_positions_for(sym)
        return jsonify(data)
    try:
        mt5.symbol_select(sym, True)
        rates = mt5.copy_rates_from_pos(sym, tf, 0, n)
        ...
        if rates is None or len(rates) < 4:
            return jsonify({"ok": False, "error": "no bars", "symbol": sym})
        # digits for sensible rounding
        info = mt5.symbol_info(sym)
        dig = int(getattr(info, "digits", 5)) if info else 5
        candles = [{
            "time": int(r["time"]),
            "open": round(float(r["open"]), dig),
            "high": round(float(r["high"]), dig),
            "low":  round(float(r["low"]),  dig),
            "close": round(float(r["close"]), dig),
        } for r in rates]
        data = {
            "ok": True,
            "symbol": sym,
            ...
            "digits": dig,
            "candles": candles,
            "positions": _r_positions_for(sym),
        }
        _candle_cache[key] = {"ts": now, "data": data}
        return jsonify(data)
    except Exception as e:
        return jsonify({"ok": False, "error": str(e), "symbol": sym})


def _r_positions_for(sym: str) -> list:
    """R-magic (20260605) open positions on `sym` — entry/SL/TP for chart markers."""
    out = []
    if not HAS_MT5:
        return out
    try:
...
@app.route("/api/r/similar")
def api_r_similar():
    """Pattern matcher: given a proposed setup, find historically similar
    closed trades and their win rate. The gate uses VERDICT to confirm or
    veto ("this setup looks like N winners last week").
    Query params: symbol, side, archetype, bias, hour (UTC int)."""
    try:
        ...
        try:    hour = int(request.args.get("hour", _dt_now_hour()))
        except Exception: hour = 0
        return jsonify(find_similar_trades(symbol, side, arch, bias, hour))
    except Exception as e:
        return jsonify({"ok": False, "error": str(e)})
...
def api_r_hour_symbol_heatmap():
    ...
    Real broker deals over the last N days (default 30, param ?days=), paired
    open→close by position_id. Each closed trade is bucketed by its ENTRY hour
    (0-23 UTC, matching hour_quality semantics) and symbol. Returns a grid the
    /r/ UI colours green/red by net P/L so time×symbol patterns are visible.

    Response:
      {ok, days, hours:[0..23], symbols:[...ordered by |net| desc],
       cells:{ "<symbol>|<hour>": {trades,wins,net} },
       symbol_totals:{symbol:{trades,wins,net}},
       hour_totals:{hour:{trades,wins,net}},
       total:{trades,wins,net}, best_cell, worst_cell}
    """
    try:
        try:
            days = max(1, min(120, int(request.args.get("days", "30"))))
        except ValueError:
            days = 30
        if not HAS_MT5:
            return jsonify({"ok": False, "error": "MT5 unavailable", "days": days,
                            "hours": list(range(24)), "symbols": [], "cells": {}})
        _ensure_mt5_alive()
        from datetime import timedelta as _td, timezone as _tz
        deals = mt5.history_deals_get(datetime.now() - _td(days=days), datetime.now()) or []
        r_deals = [d for d in deals if int(d.magic) == 20260605]

        # Pair entry (open) and exit (close) deals by position_id.
        positions = {}
        for d in r_deals:
        ...
        for pid, ds in positions.items():
            ...
            key = f"{sym}|{hour}"
            c = cells.setdefault(key, {"trades": 0, "wins": 0, "net": 0.0})
            c["trades"] += 1; c["wins"] += win; c["net"] = round(c["net"] + profit, 2)
            s = sym_tot.setdefault(sym, {"trades": 0, "wins": 0, "net": 0.0})
            s["trades"] += 1; s["wins"] += win; s["net"] = round(s["net"] + profit, 2)
            h = hour_tot.setdefault(hour, {"trades": 0, "wins": 0, "net": 0.0})
            h["trades"] += 1; h["wins"] += win; h["net"] = round(h["net"] + profit, 2)
            tot["trades"] += 1; tot["wins"] += win; tot["net"] = round(tot["net"] + profit, 2)

        # Order symbols by absolute net impact (biggest movers first).
        symbols = sorted(sym_tot.keys(), key=lambda s: -abs(sym_tot[s]["net"]))
        best_cell = worst_cell = None
        if cells:
            best_k = max(cells, key=lambda k: cells[k]["net"])
            worst_k = min(cells, key=lambda k: cells[k]["net"])
            bs, bh = best_k.rsplit("|", 1)
            ws, wh = worst_k.rsplit("|", 1)
            best_cell = {"symbol": bs, "hour": int(bh), **cells[best_k]}
            worst_cell = {"symbol": ws, "hour": int(wh), **cells[worst_k]}

        return jsonify({
            "ok": True, "days": days, "hours": list(range(24)),
            "symbols": symbols, "cells": cells,
            "symbol_totals": sym_tot, "hour_totals": hour_tot,
            "total": tot, "best_cell": best_cell, "worst_cell": worst_cell,
        })
    except Exception as e:
        import traceback
        return jsonify({"ok": False, "error": str(e), "tb": traceback.format_exc()[:300]})
...
# ── News-blackout self-heal ─────────────────────────────────────────────
# The trade_gate's news_blackout HARD check is only meaningful if it's fed
# fresh High-impact events. The calendar file is written by the (optional)
# news_straddle daemon; if that daemon isn't running the file goes stale and
# the blackout silently stops firing. This helper keeps the file fresh from
# the gate's own hot path — throttled so a network outage can't block the
# entry-scan, and fully non-fatal (always falls back to whatever is on disk).
_NEWS_REFRESH_ATTEMPT_TS = 0.0          # last time we *tried* a network refresh
_NEWS_REFRESH_THROTTLE_S = 1800         # at most one network attempt / 30 min
_NEWS_STALE_AFTER_S      = 6 * 3600     # refresh when file older than 6h


def _load_news_events_fresh() -> list:
    """Return High-impact news events for the gate, refreshing the calendar
    file when missing/stale. Never raises; never blocks more than one throttled
    fetch per 30 min."""
    global _NEWS_REFRESH_ATTEMPT_TS
    import time as _t
    news_path = Path(r"C:\Users\Radhi\MT5\friday_v3\data\news_calendar.json")
    # Determine staleness from fetched_at (preferred) or file mtime.
    age_s = None
    try:
        if news_path.exists():
            ...
            if fa:
                ...
                age_s = (_dt.now(_tz.utc) - _dt.fromisoformat(fa)).total_seconds()
    except Exception:
        data = None
    stale = (age_s is None) or (age_s > _NEWS_STALE_AFTER_S)
    now_s = _t.time()
    if stale and (now_s - _NEWS_REFRESH_ATTEMPT_TS) > _NEWS_REFRESH_THROTTLE_S:
        _NEWS_REFRESH_ATTEMPT_TS = now_s
        try:
            from friday_v3.core.news_straddle import fetch_news_calendar
            fetch_news_calendar(force=True)   # rewrites news_calendar.json
        except Exception as _e:
            print(f"[news_blackout] refresh failed (non-fatal): {_e}", flush=True)
    # Always read whatever is on disk now (fresh or last-known-good).
    try:
        return json.loads(news_path.read_text(encoding="utf-8")).get("events", [])
    except Exception:
        return []
...
def api_r_trade_gate():
    ...
    try:
        ...
        # News — self-healing: refresh the calendar if missing/stale so the
        # gate's news_blackout HARD check is always fed fresh High-impact events.
        news_events = _load_news_events_fresh()
...
# Additive endpoints (integrated from the pipflow branch). All wrapped so a
# missing dep never 500s the whole server.
...
# ── LIVE signal-system dashboard (everything in one R Native page) ─────────────
@app.route("/api/r/live")
def api_r_live():
    """Aggregate the whole live signal system: per-symbol signal + breakdown,
    learning/fade state, account, positions by system, manual review."""
    import json as _json, time as _t, glob as _glob
    COMMON = r"C:\Users\Radhi\AppData\Roaming\MetaQuotes\Terminal\Common\Files"
    V2 = r"C:\Users\Radhi\MT5\r_native_v2\data"
    out = {"ts": _t.time(), "signals": [], "learning": {}, "account": {}, "positions": {}, "manual": {}}
    for f in sorted(_glob.glob(COMMON + r"\signal_*.json")):
        try:
            import os as _os
            d = _json.loads(open(f, encoding="utf-8").read())
            d["_age"] = round(_t.time() - _os.path.getmtime(f), 1)
            out["signals"].append(d)
        except Exception:
            pass
    for fn, key in (("signal_tuning.json", "learning"),):
        try:
            out[key] = _json.loads(open(V2 + "\\" + fn, encoding="utf-8").read())
        except Exception:
            pass
    try:
        import MetaTrader5 as _mt5
        ...
        if ai:
            out["account"] = {"balance": round(ai.balance, 2), "equity": round(ai.equity, 2),
                              "profit": round(ai.profit, 2), "login": ai.login}
        from collections import defaultdict as _dd
        g = _dd(lambda: {"n": 0, "pl": 0.0})
        names = {99784: "الإشارة", 99783: "الذهب-overlay", 99782: "الدماغ", 0: "يدوي"}
        for p in (_mt5.positions_get() or []):
            k = names.get(p.magic, str(p.magic))
            g[k]["n"] += 1; g[k]["pl"] += float(p.profit)
        out["positions"] = {k: {"n": v["n"], "pl": round(v["pl"], 2)} for k, v in g.items()}
    except Exception as e:
...
@app.route("/r/live")
def page_live():
    return _serve_dashboard("live.html")
...
if __name__ == "__main__":
    ...
    import os as _os
    if _os.environ.get("DASHBOARD_ONLY") == "1":
        print("  🟢 DASHBOARD_ONLY — no evolution/agents/executor (pure dashboard, NO trading)")
    else:
    ...
    # Suppress werkzeug per-request access logging. The dashboard polls dozens of
    # /api/* endpoints every ~1s; logging each request to stderr grew embedded_brain.err
    # to 87 MB unbounded (disk-exhaustion hazard). WARNING level keeps real errors visible
    # while dropping the 200-OK access spam. Does NOT touch trading/safety logic.
