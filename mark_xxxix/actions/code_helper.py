...
def _resolve_save_path(output_path: str, language: str) -> Path:
    ...
    if output_path:
        cleaned = output_path.strip().replace("\\", "/")
        p = Path(cleaned).expanduser()
        if p.is_absolute():
            return _avoid_protected_path(p, language)

        parts = p.parts
        if parts and parts[0].lower() == "mt5":
            return _avoid_protected_path(Path.home().joinpath(*parts), language)
        if parts and parts[0].lower() in {"desktop", "downloads", "documents", "pictures", "music", "videos"}:
            base = {
                "desktop": DESKTOP,
            ...
            return _avoid_protected_path(base.joinpath(*parts[1:]), language)

        return _avoid_protected_path(PROJECT_ROOT / p, language)
    ext = ext_map.get((language or "python").lower(), ".py")
    return GENERATED_DIR / f"jarvis_code{ext}"


def _avoid_protected_path(path: Path, language: str) -> Path:
    return path
...
def _local_placeholder_code(description: str, language: str) -> str:
    lang = (language or "python").lower()
    if lang not in {"python", "py"}:
        return (
            f"// Local fallback generated because the remote code model was unavailable.\n"
            f"// Goal: {description}\n\n"
            "export function main() {\n"
            "  throw new Error('Implement this project scaffold.');\n"
            "}\n"
        )

    return f'''"""
Local fallback scaffold generated by JARVIS.
...
GATEWAY_URL = os.getenv("FRIDAY_GATEWAY_URL", "http://127.0.0.1:8799")
...
def create_tensorflow_model() -> Any:
    """Create a placeholder TensorFlow model."""
    if tf is None:
...
def ask_ollama(prompt: str, model: str = "llama3.2") -> str:
    """Send a prompt to local Ollama and return text."""
...
def gateway_health() -> dict[str, Any]:
    """Check FRIDAY local gateway health."""
    response = requests.get(f"{{GATEWAY_URL}}/health", timeout=10)
    response.raise_for_status()
    return response.json()


def dispatch_agent_task(task: str, max_agents: int = 3) -> dict[str, Any]:
    """Dispatch a task to the local FRIDAY gateway on port 8799."""
    response = requests.post(
        f"{{GATEWAY_URL}}/dispatch/run",
        json={{"task": task, "max_agents": max_agents}},
        timeout=120,
    )
    response.raise_for_status()
    return response.json()
...
def integrate_agents() -> list[AgentConfig]:
    """Define placeholder agent roles for future integration."""
    return [
        AgentConfig(name="planner", role="Break tasks into implementation steps"),
        AgentConfig(name="coder", role="Write and patch code"),
        AgentConfig(name="reviewer", role="Review output and identify risks"),
    ]


def main() -> None:
    print("FRIDAY custom AI scaffold")
    print("Gateway:", GATEWAY_URL)
    print("Ollama:", OLLAMA_URL)
    print("Agents:", integrate_agents())
...
def _write_local_fallback(description: str, language: str, output_path: str) -> tuple[str, Path]:
    code = _local_placeholder_code(description, language)
...
def _build(description, language, output_path, args, timeout, speak=None, player=None) -> str:
    ...
    try:
        ...
        if _is_quota_error(e):
            try:
                code, path = _write_local_fallback(description, lang, output_path)
                print(f"[Code] ✅ Local fallback written: {path}")
            except Exception as fallback_error:
                msg = f"Remote quota exceeded and local fallback failed: {fallback_error}"
                if speak:
                    speak(msg)
                return msg
        else:
...
def _write_action(description, language, output_path, player) -> str:
    ...
    try:
        ...
        if _is_quota_error(e):
            try:
                code, path = _write_local_fallback(description, language, output_path)
                print(f"[Code] ✅ Local fallback written: {path}")
                return (
                    "Remote code model quota was exceeded, so I used the local "
                    f"fallback scaffold. Saved to: {path}\n\nPreview:\n{_preview(code)}"
                )
            except Exception as fallback_error:
                return f"Remote quota exceeded and local fallback failed: {fallback_error}"
