...
def _is_safe_path(target: Path, operation: str = "read") -> bool:
    allowed, _reason = is_path_allowed(target, operation)
    return allowed

def _deny_message(target: Path, operation: str = "read") -> str:
    _allowed, reason = is_path_allowed(target, operation)
    return f"Access denied ({reason}): {target}"
...
def _resolve_path(raw: str) -> Path:
    ...
    if parts:
        ...
        if first in shortcuts:
            return shortcuts[first].joinpath(*parts[1:])
        if first == "mt5":
...
def list_files(path: str = "desktop", show_hidden: bool = False) -> str:
    try:
        ...
        if not _is_safe_path(target, "read"):
            return _deny_message(target, "read")
...
def create_file(path: str, name: str = "", content: str = "") -> str:
    try:
        ...
        if not _is_safe_path(target, "write"):
            return _deny_message(target, "write")
        if _is_protected_file(target):
            return f"Protected file, cannot write: {target}"
...
def delete_file(path: str, name: str = "") -> str:
    try:
        base   = _resolve_path(path)
        target = (base / name) if name else base
        if not _is_safe_path(target, "delete"):
            return _deny_message(target, "delete")
        if not target.exists():
...
def move_file(path: str, name: str = "", destination: str = "") -> str:
    try:
        ...
        if not _is_safe_path(src, "move"):
            return _deny_message(src, "move")
        if not _is_safe_path(dst, "write"):
            return _deny_message(dst, "write")
...
def copy_file(path: str, name: str = "", destination: str = "") -> str:
    try:
        ...
        if dst is None:
            return "No destination specified."
        if not _is_safe_path(src, "read"):
            return _deny_message(src, "read")
        if not _is_safe_path(dst, "write"):
            return _deny_message(dst, "write")
...
def rename_file(path: str, name: str = "", new_name: str = "") -> str:
    try:
        base     = _resolve_path(path)
        target   = (base / name) if name else base
        if not _is_safe_path(target, "move"):
            return _deny_message(target, "move")
        if not target.exists():
            return f"Not found: {target.name}"
        if not new_name:
        ...
        if not _is_safe_path(new_path, "write"):
            return _deny_message(new_path, "write")
        if new_path.exists():
...
def write_file(path: str, name: str = "", content: str = "",
    ...
    try:
        ...
        if target.exists() and target.is_dir():
            return f"Target is a directory, not a file: {target}"
        target.parent.mkdir(parents=True, exist_ok=True)
...
def find_files(name: str = "", extension: str = "",
    ...
    try:
        ...
        if not _is_safe_path(search_path, "read"):
            return _deny_message(search_path, "read")
        ...
        for item in search_path.rglob("*"):
            ...
            if not item.is_file():
                continue
            if not _is_safe_path(item, "read") or _is_protected_file(item):
                continue
...
def organize_desktop() -> str:
    ...
    try:
        if not _is_safe_path(desktop, "move"):
            return _deny_message(desktop, "move")
...
def file_controller(
    ...
    if action == "write":
        path = params.get("path") or params.get("output_path") or params.get("file_path") or "desktop"
    else:
        path = params.get("path") or params.get("output_path") or params.get("file_path") or "desktop"
    ...
    try:
        if action in {"permissions", "runtime_status"}:
            return format_runtime_snapshot(detail=True)
