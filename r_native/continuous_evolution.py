...
# ── LIVE-VALIDATION GATE (added 2026-06-30) ──────────────────────────
# A genome may NOT be full-deployed to live trading on backtest score alone.
# It must show >= MIN_LIVE_TRADES real closed trades (friday.db, R magic) with
# aggregate net >= 0. Below that it may only TRIAL at min-lot, never replace
# the live genome. Backtest score is necessary but NOT sufficient.
FRIDAY_DB        = Path(r"C:\Users\Radhi\MT5\data\friday.db")
R_LIVE_MAGIC     = 20260605          # R Executor live magic (r_native.actions.R_MAGIC)
MIN_LIVE_TRADES  = 10                # real closed trades required for full deploy
MIN_LIVE_NET     = 0.0               # aggregate net (pnl+comm+swap) must be >= this


def _live_trade_record(symbol: str) -> dict:
    """Read-only: real closed-trade evidence for `symbol` under the R live magic.

    Returns {"n": int, "net": float}. Fails open to {"n":0,"net":0.0} on any
    error so a DB hiccup never *grants* a deploy (it only ever withholds one)."""
    try:
        if not FRIDAY_DB.exists():
            return {"n": 0, "net": 0.0}
        import sqlite3
        try:
            c = sqlite3.connect(f"file:{FRIDAY_DB}?mode=ro", uri=True)
        except Exception:
            c = sqlite3.connect(str(FRIDAY_DB))
        try:
            row = c.execute(
                "select count(*), "
                "coalesce(sum(coalesce(pnl,0)+coalesce(commission,0)+coalesce(swap,0)),0) "
                "from trades where magic=? and symbol=? and exit is not null",
                (R_LIVE_MAGIC, symbol),
            ).fetchone()
        finally:
            c.close()
...
def _mark_trial(symbol: str, genome_id: str, tf: str, top: dict,
                score: float, live: dict) -> None:
    """Record a min-lot TRIAL intent in the symbol config WITHOUT replacing the
    live `deployed_genome`. Writes to a separate `trial_genome` key so a
    backtest-only gene can never reach full live lot. Read by nothing yet — the
    executor min-lot trial hook is a follow-up (see deploy_notes)."""
    try:
        ...
        if cfg_path.exists():
            try:
                ...
                cfg = {}
        cfg["trial_genome"] = {
            "id": genome_id, "tf": tf, "score": score,
            "min_lot_only": True,
            "live_trades": live.get("n", 0), "live_net": live.get("net", 0.0),
            "marked_at": datetime.now(timezone.utc).isoformat(),
            "reason": "backtest-passed; awaiting >=%d live trades net>=%s for full deploy"
                      % (MIN_LIVE_TRADES, MIN_LIVE_NET),
        }
...
def _maybe_auto_deploy(symbol: str, summary: dict, threshold: float,
    ...
    # ── LIVE-VALIDATION GATE (2026-06-30) ───────────────────────────────
    # Backtest score beating current by `threshold` is necessary but NOT
    # sufficient. Require real forward evidence: >= MIN_LIVE_TRADES closed
    # live trades (friday.db, R magic) with aggregate net >= MIN_LIVE_NET.
    # Below that → TRIAL at min-lot only; do NOT full-deploy. This stops the
    # GA from pushing overfit genes (live_trades=0) straight to full live lot.
    live = _live_trade_record(symbol)
    if live["n"] < MIN_LIVE_TRADES or live["net"] < MIN_LIVE_NET:
        _mark_trial(symbol, new_id, summary.get("timeframe", "M5"),
                    top, new_score, live)
        _log(f"🧪 TRIAL-ONLY {new_id} on {symbol}: backtest passed "
             f"(score {new_score:.1f} vs {cur_score:.1f}, +{delta:.1f}) but live "
             f"evidence insufficient (trades={live['n']}<{MIN_LIVE_TRADES} or "
             f"net={live['net']:.2f}<{MIN_LIVE_NET}); marked min-lot trial, "
             f"NOT full-deployed.")
        return {"deployed": False, "trial": True, "id": new_id,
                "reason": f"live-validation: {live['n']} live trades "
                          f"(need >={MIN_LIVE_TRADES}) net {live['net']:.2f} "
                          f"(need >={MIN_LIVE_NET}); min-lot trial only",
                "live_trades": live["n"], "live_net": live["net"],
                "new_score": new_score, "delta": delta}
    _log(f"✅ LIVE-VALIDATED {new_id} on {symbol}: {live['n']} live trades, "
         f"net {live['net']:.2f} — clearing for full deploy.")
