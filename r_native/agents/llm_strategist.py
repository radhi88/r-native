    ...
    def tick(self):
        ...
        if not result.get("ok"):
            ...
            emit_insight(self.name, "WARN",
                         ...
                         f"last_error: {err}")
            # Persist a DEGRADED status so the UI shows the dead backend
            # instead of the last successful run's stale verdict (which made
            # 'autonomous ON' appear healthy while no LLM was reachable).
            # We do NOT clobber the prior assessment/recommendations — those
            # stay as the last-known-good — but we add explicit degraded
            # fields the GUI keys off. No trading logic touched.
            try:
                prev = {}
                if STRATEGIST_STATE.exists():
                    prev = json.loads(STRATEGIST_STATE.read_text(encoding="utf-8"))
                prev["llm_degraded"] = True
                prev["last_attempt"] = datetime.now(timezone.utc).isoformat()
                prev["last_error"] = err or f"no LLM backend (tried [{tried_str}])"
                prev["tried"] = tried_str
                prev["autonomous"] = self._autonomous
                STRATEGIST_STATE.parent.mkdir(parents=True, exist_ok=True)
                STRATEGIST_STATE.write_text(
