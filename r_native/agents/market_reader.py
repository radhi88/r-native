    ...
    def _current_genome(self, symbol: str) -> dict | None:
        ...
        try:
            ...
            if isinstance(dg, dict):
                return dg
            if isinstance(dg, str) and dg:           # deployed_genome may be an ID string —
                for s in cfg.get("ga_strategies", []):   # resolve it to its strategy dict
                    if isinstance(s, dict) and s.get("id") == dg:
                        return s
                return None
            return None
        except Exception:
