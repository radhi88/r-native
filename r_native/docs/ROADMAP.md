...
- [x] P1 · ARCH · Worker heartbeat server on `127.0.0.1:7711`
      ...
      EST: 45 min
      DONE 2026-07-13: `r_native/heartbeat_server.py` (Flask daemon thread, loopback-only,
      GET /heartbeat + POST /shutdown + POST /restart + RSS watchdog) wired into
      `r_native/app.py` main(). Verified: GET /heartbeat → ok+pid+ram_mb+uptime_sec.
      (Twin of r_desktop/ROADMAP.md entry.)
...
- [x] P1 · ARCH · RNativeLauncher skeleton (tray + status window + subprocess.Popen)
      ...
      EST: 90 min
      DONE 2026-07-13: `r_native_launcher/launcher.py` (435 lines) — config-driven
      (`launcher_config.json`), rotating `launcher.log`, dead = process-exit OR 3×
      heartbeat miss OR wants_restart, own-child-tree kill only, exponential backoff
      capped at 6 restarts/hour then HALT. pystray tray (console fallback), tkinter
      status window, CLI --console/--dry-run. Verified restart-on-death in dry-run:
      external kill → respawn after 2s backoff → clean exit. (Twin of r_desktop copy.)
...
- [x] P1 · MEMORY · Worker cleanup after GA campaign + LRU bar cache
      ...
      EST: 50 min
      DONE 2026-07-14: `r_native/bar_cache.py` rewritten as SYMBOL-capped LRU
      (OrderedDict symbol→{(tf,n)→(ts,bars)}, MAX_SYMBOLS=5, 90s staleness TTL,
      failed fetches never cached) and routed through the two heaviest call
      sites: `scanner.scan_symbol_tf_archetype` (1 fetch instead of ~7/archetype)
      and `genetic_engine._evaluate_genome_worker` (1 fetch per pool worker
      instead of per genome eval). `CampaignWorker.run` now clears
      `engine.vault=[]` + `engine.elites=[]` after persisting to disk;
      `_on_campaign_done` already dropped the QThread + `gc.collect()` +
      `shrink_to(keep=5)`. Verified offline: 8 symbols in → 5 remain MRU-order,
      get() refreshes MRU, TTL miss, fetch dedup — 18/18 checks.
...
- [x] P2 · ARCH · Worker auto-restart at 500MB RSS threshold
      ...
      EST: 30 min
      DONE 2026-07-14: threshold now configurable — env `R_NATIVE_RSS_LIMIT_MB`
      > `data/r_native/settings.json` `"rss_limit_mb"` > default 500
      (`heartbeat_server.resolve_rss_limit_mb`). Breach → `wants_restart=true`
      in /heartbeat (launcher respawns on it) → 60s grace → graceful shutdown
      hook (Qt quit) → `os._exit(0)` hard fallback 10s later. `app.py` no longer
      hardcodes 1800; settings.json carries rss_limit_mb=1800 until the
      two-process split lands (monolith legitimately boots ~515MB). Verified
      offline with injected fake RSS: breach → wants_restart true + hook fired;
      below threshold → stays false.
...
- [x] P2 · ARCH · Launcher control RPC (restart-worker / kill-worker from tray menu)
      ...
      EST: 40 min
      DONE 2026-07-14: line-JSON RPC on 127.0.0.1:7712; tray routes via RPC,
      threads per request (no UI freeze); `--send CMD` external client.
      Verified dry-run: status/restart-worker (kill+respawn)/quit → exit 0.
...
- [x] P3 · ARCH · Hot-swap workers (atomic .exe.new replacement)
      ...
      EST: 60 min
      DONE 2026-07-14: pre-spawn `check_hot_swap()` (>1MB + size-stable 1s) →
      current→.bak, .new→current; module-mode skips; RPC `swap-check` +
      `--test-swap`. Verified: 1.5MB .new swapped in, .bak kept; 100B rejected.
