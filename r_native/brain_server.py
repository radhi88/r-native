...
@app.route("/api/r/agents/start_all", methods=["POST"])
def api_r_agents_start_all():
    """Start ALL 31 agents (idempotent). Called by the onboarding Approve button so the
    full agent fleet runs autonomously even when the brain was launched embedded."""
    try:
        from r_native.agents.orchestrator import start_all, list_agents
        start_all()
        ags = list_agents()
        running = sum(1 for a in ags if a.get("running") or a.get("thread_alive"))
        return jsonify({"ok": True, "started": running, "total": len(ags)})
    except Exception as e:
        return jsonify({"ok": False, "error": str(e)})
...
def api_r_similar():
    ...
    closed trades and their win rate. The gate already consults this in-process
    (trade_gate.py) to nudge confidence ("this setup looks like N winners last
    week"); this endpoint exposes the same verdict for inspection on the UI.
    Query params: symbol, side, archetype, bias, hour (UTC int, defaults to now)."""
    try:
        ...
        try:    hour = int(request.args.get("hour", datetime.utcnow().hour))
        except Exception: hour = datetime.utcnow().hour
