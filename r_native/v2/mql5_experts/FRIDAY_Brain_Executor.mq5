...
input group  "═══ OUR SON gate (v2 brain) ═══"
input bool    InpUseSonGate        = true;              // gate trades on our son's P(win)
input double  InpSonMinPwin        = 0.50;              // min P(win) to allow a trade
input string  InpSonStatusFile     = "son_status.json"; // unified_trader mirrors it to Common/Files
input int     InpSonMaxAgeSec      = 45;                // ignore son verdict older than this (fail-open)

input group  "═══ Night-trade block (reflection discipline) ═══"
// The strongest measured FRIDAY edge: excluding 22:00-08:00 flipped the user's
// real manual record from -$18 to +$904 (PF 3.36). Pauses NEW entries in the
// window only — CANCEL_ALL and open-position management are unaffected. Mirrors
// reflection/strategy.json night_trade_block. Hours are BROKER SERVER time (your
// chart clock) via TimeTradeServer() — the frame the edge was measured in, and
// DST-proof (no UTC offset to maintain as the broker flips EET/EEST).
input bool    InpNightBlock        = true;   // pause NEW entries during the night window
input int     InpNightStartHour     = 22;     // block from this SERVER hour (inclusive)...
input int     InpNightEndHour       = 8;      // ...until this SERVER hour (exclusive); wraps midnight
...
//+------------------------------------------------------------------+
//| OUR SON gate — read son_status.json (written by unified_trader)  |
//| Returns: allowed?  + fills out_pwin / out_stage.                 |
//| Fail-OPEN: if file missing/stale/unparseable → allow.            |
//+------------------------------------------------------------------+
bool SonGateAllows(double &out_pwin, string &out_stage) {
   out_pwin = 0.5; out_stage = "?";
   if (!InpUseSonGate) return true;
   string st;
   if (!ReadCommonFile(InpSonStatusFile, st)) return true;   // no file → allow
   long ts_epoch = 0;
   // son_status ts is ISO string; we rely on file recency via a numeric guard:
   // unified_trader rewrites it every loop, so staleness ≈ file age is enough.
   out_stage = JsonStr(st, "stage");
   out_pwin  = JsonNum(st, "p_win");
   string side = JsonStr(st, "side");
   // If son is actively FIRING or its P(win) clears the bar → allow.
   if (out_stage == "FIRING") return true;
   if (out_pwin >= InpSonMinPwin) return true;
   // Son is blocking (ML_BLOCK / WAITING / FROZEN / low P(win)) → veto.
   return false;
}

//+------------------------------------------------------------------+
//| Night-trade block — BROKER SERVER time (TimeTradeServer), wraps. |
//| Returns true when NEW entries should be paused right now.        |
//+------------------------------------------------------------------+
bool InNightWindow() {
   if (!InpNightBlock) return false;
   MqlDateTime dt;
   TimeToStruct(TimeTradeServer(), dt);   // broker server time — DST-proof
   int h = dt.hour;
   int s = InpNightStartHour, e = InpNightEndHour;
   if (s == e) return false;
   if (s < e)  return (h >= s && h < e);
   return (h >= s || h < e);   // wraps midnight (e.g. 22 → 8)
}
...
void ProcessBrainJson() {
   ...
   if (action == "PLACE" && entry > 0 && sl > 0) {
      // ── NIGHT-TRADE BLOCK (reflection discipline) — pause NEW entries in the
      // 22:00-08:00 UTC window (the strongest measured FRIDAY edge). CANCEL_ALL
      // and management still run; only fresh risk is held. Consumes the epoch so
      // it doesn't re-check the same brain order every timer tick.
      if (InNightWindow()) {
         MqlDateTime _gdt; TimeToStruct(TimeTradeServer(), _gdt);
         g_last_reason = StringFormat("🌙 night block %02d:00-%02d:00 srv (h=%02d)",
                                       InpNightStartHour, InpNightEndHour, _gdt.hour);
         g_last_epoch_processed = epoch;
         RenderDashboard(txt);
         return;
      }
      // ── OUR SON gate: only trade when the v2 brain's P(win) agrees ──
      double son_pwin; string son_stage;
      if (!SonGateAllows(son_pwin, son_stage)) {
         g_last_reason = StringFormat("SON veto: %s P(win)=%.2f < %.2f",
                                       son_stage, son_pwin, InpSonMinPwin);
         g_last_epoch_processed = epoch;   // consume epoch so we don't re-check spam
         RenderDashboard(txt);
         return;
      }
...
int OnInit() {
   ...
   Print("  Night:   ", (InpNightBlock
         ? StringFormat("BLOCK %02d:00-%02d:00 server-time (new entries paused)",
                        InpNightStartHour, InpNightEndHour)
         : "off"));
   Print("═══════════════════════════════════════════════");
