...
from PySide6.QtWidgets import (QApplication, QMainWindow, QWidget, QVBoxLayout, QHBoxLayout,
                               ...
                               QStackedWidget, QInputDialog, QDockWidget, QToolButton)
...
GOLD_BRIGHT   = "#f7b341"   # lighter gold — hover tints / gradient ends
GOLD_BG       = "#3d2c0a"   # amber badge/banner background (warning tint)
...
def apply_dark_theme(app):
    ...
    app.setStyleSheet(f"""
        ...
        QPushButton[role="primary"]:hover  {{ background: {GOLD_BRIGHT}; }}
...
class CampaignWorker(QThread):
    ...
    def run(self):
        ...
        # ── H.4: vault now lives on disk (vault.json + per-symbol config) —
        # drop the in-RAM copies so the worker shrinks back after a campaign.
        # (_on_campaign_done then drops this QThread + gc.collect()s.)
        try:
            engine.vault = []
            engine.elites = []
        except Exception:
...
class RNativeMain(QMainWindow):
    def __init__(self):
        ...
        # ── Embedded side terminal (شاشة تيرنمال جانبي) — real interactive
        # PowerShell living inside the app, docked to the right edge. Lets the
        # user run git/python/log-tail commands without leaving R Native. ──
        self._setup_terminal_dock()
        # Ctrl+` toggles the terminal (VS Code muscle memory)
        # Ctrl+K opens the command palette — كل تبويب وأمر تحت إصبعك
        try:
    ...
    def _refresh_hero_card(self) -> None:
        ...
        # Gate (last action snippet) — elide with ellipsis + full text on hover,
        # instead of the old silent 20-char chop that hid the rest ("...non").
        if last_action and last_action != "—":
            gate_text = (last_action[:24] + "…") if len(last_action) > 25 else last_action
            self.hero_gate.setToolTip(last_action)
        else:
            gate_text = "—"
            self.hero_gate.setToolTip("")
    ...
    def _build_header(self):
        ...
        mode_lbl = QLabel("الوضع / MODE:")
        mode_lbl.setStyleSheet(f"color: {TEXT_MUTED}; font-size: 10px; font-weight: 600; "
                               f"letter-spacing: 1px; margin-right: 2px;")
        h.addWidget(mode_lbl)
        # Segmented toggle — one container, active mode filled GOLD
        seg = QFrame()
        seg.setStyleSheet(f"QFrame {{ background: {BG_1}; border: 1px solid {BORDER}; "
                          f"border-radius: 6px; }}")
        seg_l = QHBoxLayout(seg); seg_l.setContentsMargins(3, 3, 3, 3); seg_l.setSpacing(2)
    ...
    def _style_mode_button(self, b, active):
        if active:
            b.setStyleSheet(f"background: {GOLD}; color: {BG_0}; padding: 6px 14px; "
                            f"border: none; border-radius: 4px; font-weight: bold;")
        else:
            b.setStyleSheet(f"background: transparent; color: {MUTED}; padding: 6px 14px; "
                            f"border: none; border-radius: 4px; font-weight: bold;")
    ...
    def _build_command_sidebar(self):
        ...
        # Utility actions collapsed into ONE dropdown (was 5 stacked buttons —
        # frees vertical space so the sidebar breathes; user: "نخليه قائمة منسدلة").
        # The primary RUN/PAUSE/STOP controls stay as big buttons below.
        tools_btn = QPushButton("⚙  أدوات  ▾")
        tools_btn.setToolTip("الإعدادات · الخزنة · الطرفية · الاتصال · الدعم")
        ...
        for lbl, handler in [("⚙ Settings", self._open_settings),
            ...
            tools_menu.addAction(lbl, handler)
        tools_btn.setMenu(tools_menu)
        v.addWidget(tools_btn)
        ...
        self.live_activity.setStyleSheet(
            f"QPlainTextEdit {{ background: {BG_0}; color: {TEXT}; border: 1px solid {BORDER};"
        ...
        self.gate_status_frame.setStyleSheet(
            f"QFrame {{ background: {BG_0}; border: 1px solid {BORDER}; border-radius: 4px; padding: 6px; }}")
    ...
    def _open_command_palette(self):
        """Ctrl+K — fuzzy launcher over every tab + action (command_palette.py)."""
        try:
            from r_native.command_palette import CommandPalette
        except Exception as e:
            print(f"[palette] unavailable: {e}", flush=True)
            return
        cmds = []
        tabs = getattr(self, "main_tabs", None)
        if tabs is not None:
            for i in range(tabs.count()):
                cmds.append((f"⇥  تبويب   {tabs.tabText(i)}",
                             lambda i=i: (self.center_stack.setCurrentIndex(0),
                                          tabs.setCurrentIndex(i))))
        cmds += [
            ("▶  RUN — تشغيل الفحص", self._start_scan),
            ("⏸  PAUSE — إيقاف مؤقت", self._toggle_pause),
            ("⏹  STOP — إيقاف الفحص", self._stop_scan),
            ("🔁  REPEAT — تكرار", self._toggle_repeat),
            ("📥  QUEUE — قائمة الانتظار", self._show_queue),
            ("⚙  Settings — الإعدادات", self._open_settings),
            ("📁  Vault — مجلد الخزنة", self._open_vault_folder),
            ("🖥  Terminal — الطرفية (Ctrl+`)", self._toggle_terminal),
            ("🔋  Connection — حالة الاتصال", self._show_connection),
            ("🆘  Request Support — طلب دعم", self._request_remote_support),
        ]
    ...
    def _bg_fetch(self, work, on_ok):
        """Run blocking work() in a daemon thread, then deliver its result to
        on_ok() on the Qt main thread via QTimer.singleShot.

        Root-cause fix for the startup freeze: network (urlopen) and subprocess
        (tasklist) calls were running directly on the GUI thread inside 5s
        refresh timers. Each could block for seconds; stacked together they
        exceeded the 5s interval, so the event loop never went idle and the
        window never painted (Responding=False). Keeping all blocking I/O on a
        worker thread guarantees the UI stays live."""
        import threading
        def _runner():
            try:
                result = work()
        ...
        threading.Thread(target=_runner, daemon=True,
                         name="rnative-bg-fetch").start()

    def _refresh_champions_bar(self):
        import urllib.request, json
        def _work():
            with urllib.request.urlopen(
                    "http://127.0.0.1:5055/api/r/genomes/active", timeout=2) as r:
                return json.loads(r.read().decode())
        self._bg_fetch(_work, self._render_champions_bar)

    def _render_champions_bar(self, d):
        if not d:
            return
    ...
    def _build_mission_tab(self):
        ...
        try:
            ...
            btn.setStyleSheet(
                ...
                f"QPushButton:hover {{ background: {GOLD_DIM}; }}")
    ...
    def _build_active_tab(self):
        ...
        header.setSpacing(6)
        self._active_daemon_pill = QLabel("○ daemons…")
        self._active_daemon_pill.setToolTip(
            "نبض الخدمات الخلفية — background daemons heartbeat\n"
            "lin=lineage · asym=asymmetry · mon=monster · con=contender\n"
            "● alive · ○ down")
        ...
        btn_refresh.setToolTip(
            "تحديث فوري لجدول المراكز — refresh now\n"
            "(يتحدّث تلقائياً كل 5 ثوانٍ)")
        ...
        # Empty state — shown instead of a bare 0-row table (toggled by
        # _apply_active_table; the table itself stays in place underneath).
        self._active_empty_lbl = QLabel(
            "لا مراكز مفتوحة الآن — القناص ينتظر إشارة صالحة")
        self._active_empty_lbl.setAlignment(Qt.AlignCenter)
        self._active_empty_lbl.setWordWrap(True)
        self._active_empty_lbl.setStyleSheet(
            f"color: {TEXT_MUTED}; font-size: 12px; padding: 18px 8px;"
            f" background: {BG_1}; border: 1px dashed {BORDER};"
            f" border-radius: 8px;")
        self._active_empty_lbl.setVisible(False)
        v.addWidget(self._active_empty_lbl)
    ...
    def _refresh_active_table(self):
        ...
        import json as _j
        def _work():
            with _ur.urlopen("http://127.0.0.1:5055/api/r/genomes/active",
                              timeout=2) as r:
                return _j.loads(r.read().decode())
        self._bg_fetch(_work, self._apply_active_table)

    def _apply_active_table(self, d):
        if not d:
            self._active_footer.setText("err: brain offline (:5055)")
            return
        ...
        # Toggle the friendly empty-state message when there is nothing to show
        if getattr(self, "_active_empty_lbl", None) is not None:
            self._active_empty_lbl.setVisible(len(rows) == 0)
    ...
    def _refresh_daemon_pill(self):
        """Probe daemons by scanning running python.exe via tasklist /V /FO CSV.
        The (slow) tasklist call runs off the GUI thread so the window never
        freezes; results are applied on the main thread in _apply_daemon_pill."""
        import subprocess
        def _work():
            r = subprocess.run(
            ...
            return r.stdout or ""
        self._bg_fetch(_work, self._apply_daemon_pill)

    def _apply_daemon_pill(self, blob):
        if blob is None:
            return
        try:
            # tasklist's CSV doesn't have CommandLine — we infer from
            # launcher_pids.json + check if the PID is in tasklist's listed
            # PIDs (just confirms process exists).
            import json as _j
            from pathlib import Path as _P
    ...
    def _build_center(self):
        ...
        tabs = QTabWidget()
        self.main_tabs = tabs   # exposed for the Ctrl+K command palette
        ...
        tabs.addTab(self._build_hof_tab(),      "🏆 HOF")
        tabs.addTab(self._build_advisors_tab(), "🤖 ADVISORS")
        tabs.addTab(self._build_son_tab(),      "🧬 OUR SON")
        # 🧔 RADHI — the Radhi-clone council: live deliberation vs your decoded DNA
        # + the OOS-validated champions vault (built 2026-05-31).
        try:
            from r_native.radhi_panel import RadhiPanel
            tabs.addTab(RadhiPanel(), "🧔 RADHI")
        except Exception as _e:
            print(f"[radhi_panel] tab skipped: {_e}", flush=True)
        # 🖥 R TRADER — the :8020 unified gateway embedded WHOLE (القمرة + مخ
        # الشركة /rt/brain بكل مميزاته)، مع إقلاع ذاتي لو كانت البوّابة مطفأة.
        try:
            from r_native.rtrader_tab import RTraderTab
            tabs.addTab(RTraderTab(), "🖥 R TRADER")
        except Exception as _e:
            print(f"[rtrader_tab] tab skipped: {_e}", flush=True)
        # Compact, non-overflowing tab bar — full descriptive tooltips compensate
        # for the shortened labels (HOF = Hall of Fame, ADVISORS = AI Advisors).
        tabs.setElideMode(Qt.ElideNone)
        tabs.setUsesScrollButtons(True)          # 11 tabs — graceful scroll if tight
        tabs.tabBar().setExpanding(False)
        tabs.setMovable(True)                    # user can reorder tabs to taste
        tabs.setDocumentMode(True)               # flatter, cleaner tab bar
        for _i, _tip in enumerate(["Mission control", "Active positions", "GA campaign",
                                    "Strategy vault", "Live execution", "Gene pool",
                                    "Hall of Fame", "AI Advisors", "Our Son (v2 live system)",
                                    "Radhi clone council + champions vault",
                                    "R Trader gateway :8020 — القمرة + مخ الشركة الحي"]):
            if _i < tabs.count(): tabs.setTabToolTip(_i, _tip)
        ...
        # Bottom action bar — decongested 2026-05-30. Was 10 buttons crammed in one
        # row (text truncated, "محشور حشر"). Now: 3 PRIMARY actions stay as buttons;
        # the 6 secondary/destructive actions move into a "⋯ MORE" dropdown menu
        # ("نخليه قائمة منسدلة") so the row breathes and destructive ops aren't
        # one fat-finger away.
        ...
        # ⋯ MORE — secondary + maintenance + destructive actions, tucked away
        more_btn = QToolButton()
        more_btn.setText("⋯ MORE")
        more_btn.setToolTip(
            "إجراءات إضافية — more actions:\n"
            "OPTIMIZE · RETRAIN · DEDUPE · PURGE · DELETE genome · CLOSE ALL")
        more_btn.setPopupMode(QToolButton.InstantPopup)
        more_btn.setToolButtonStyle(Qt.ToolButtonTextOnly)
        more_btn.setProperty("role", "")
        more_menu = QMenu(more_btn)
        for lbl, handler, danger in [("⚙ OPTIMIZE", self._action_optimize, False),
                                      ("🔁 RETRAIN", self._action_retrain, False),
                                      ("🧹 DEDUPE", self._action_dedupe, False),
                                      ("🗑 PURGE", self._action_purge, False),
                                      (None, None, False),          # separator
                                      ("✖ DELETE genome", self._action_delete, True),
                                      ("⛔ CLOSE ALL positions", self._action_close_all, True)]:
            if lbl is None:
                more_menu.addSeparator(); continue
            act = more_menu.addAction(lbl)
            act.triggered.connect(handler)
            if danger:
                act.setProperty("danger", True)
        more_btn.setMenu(more_menu)
        more_btn.setStyleSheet(
            f"QToolButton {{ background: {BG_2}; color: {TEXT}; border: 1px solid {BORDER};"
            f"  border-radius: 8px; padding: 8px 14px; font-weight: 600; }}"
            f"QToolButton:hover {{ border-color: {GOLD}; color: {GOLD}; }}"
            f"QToolButton::menu-indicator {{ image: none; }}")
        action_bar.addWidget(more_btn)
        ...
        self._auto_evo_status_lbl.setStyleSheet(
            f"color: {TEXT_MUTED}; font-size: 11px; padding-left: 6px;")
    ...
    def _build_campaign_tab(self):
        ...
        # ── ① Full-market scan — multi-asset sweep driven by ▶ RUN. ───────
        sec1 = QLabel("① فحص السوق الكامل — full-market scan")
        sec1.setStyleSheet(f"color: {GOLD}; font-weight: 700; font-size: 13px;")
        v.addWidget(sec1)

        # ASSETS — grouped by class so FX / metals / indices / crypto / oil read
        # as distinct blocks instead of one flat 18-box wall. Every checkbox object
        # and its self.symbol_checks[sym] registration is preserved 1:1 — the scan
        # reads self.symbol_checks, so only the LAYOUT is reorganised here.
        assets_box = QGroupBox("ASSETS"); assets_box.setStyleSheet(f"QGroupBox {{ color: {GOLD}; }}")
        av = QVBoxLayout(assets_box); av.setSpacing(6)
        self.symbol_checks = {}
        ASSET_GROUPS = [
            ("💱 عملات / FX", ["EURUSDm","GBPUSDm","AUDUSDm","USDJPYm","USDCADm",
                                "USDCHFm","EURGBPm","GBPJPYm","EURJPYm","GBPAUDm"]),
            ("🥇 معادن / Metals",   ["XAUUSDm","XAGUSDm"]),
            ("📈 مؤشرات / Indices", ["US30m","USTECm","US500m","DE30m"]),
            ("₿ كريبتو / Crypto",   ["BTCUSDm"]),
            ("🛢 سلع / Commodities", ["USOILm"]),
        ]
        for _title, _syms in ASSET_GROUPS:
            hdr = QLabel(_title)
            hdr.setStyleSheet(f"color: {GOLD}; font-weight: 600; font-size: 12px; padding-top: 2px;")
            av.addWidget(hdr)
        ...
        tg.setHorizontalSpacing(12); tg.setVerticalSpacing(6)
        self.tf_checks = {}
        ...
        pg.setColumnMinimumWidth(0, 92)   # label column — "Spread mult" must never clip
        pg.setColumnStretch(0, 0); pg.setColumnStretch(1, 1)
        self.param_inputs = {}
        ...
        # ── ② Single-symbol GA campaign — a DIFFERENT feature from the scan above:
        # it evolves strategies for ONE symbol/TF via _start_campaign (not the
        # multi-asset sweep). Gold divider + title so the two don't read as dupes.
        div = QFrame(); div.setFrameShape(QFrame.HLine)
        div.setStyleSheet(f"background: {GOLD}; max-height: 2px; border: none;")
        v.addWidget(div)
        sec2 = QLabel("② حملة GA على رمز واحد — evolve one symbol/TF")
        sec2.setStyleSheet(f"color: {GOLD}; font-weight: 700; font-size: 13px; padding-top: 4px;")
        v.addWidget(sec2)
        ...
        gg.setHorizontalSpacing(8); gg.setVerticalSpacing(8)
        # Clean 4-column form row: label / field / label / field, equal label widths
        gg.setColumnMinimumWidth(0, 92); gg.setColumnMinimumWidth(2, 92)
    ...
    def _build_vault_tab(self):
        ...
        # Section title — matches the GOLD letter-spaced header used elsewhere
        vault_hdr = QLabel("💎 VAULT — الخزنة")
        vault_hdr.setStyleSheet(f"color: {GOLD}; font-size: 10px; font-weight: 700; letter-spacing: 2px;")
        v.addWidget(vault_hdr)
    ...
    def _build_live_tab(self):
        ...
        def _section_hdr(text: str, hint: str = "") -> QLabel:
            # GOLD section header + muted hint — replaces the old plain one-line
            # info dumps that blended into the tables.
            lbl = QLabel(
                f"<span style='color:{GOLD}; font-weight:700; letter-spacing:2px;"
                f" font-size:11px'>{text}</span>"
                + (f"  <span style='color:{TEXT_MUTED}; font-size:10px'>{hint}</span>"
                   if hint else ""))
        ...
        v.addWidget(_section_hdr("📦 OPEN R POSITIONS",
                                 "click row → inspect genome · double-click or Why? → full reasoning"))
        v.addWidget(self.live_table, 1)

        # Per-symbol config display
        v.addWidget(_section_hdr("📋 PER-SYMBOL CONFIGS", "click a row to inspect"))
    ...
    def _build_son_tab(self):
        """🧬 OUR SON — live widget cockpit (son_cockpit.SonCockpit).

        2026-05-30: replaced the old QPlainTextEdit text-dump with real widgets
        (per-symbol cards + BUY/SELL condition pills + decisions feed). The
        cockpit owns its own timer and SKIPS refreshing while the tab is hidden
        (efficiency). Fallback label keeps the tab alive if import ever fails."""
        try:
            from r_native.son_cockpit import SonCockpit
            self.son_cockpit = SonCockpit()
            return self.son_cockpit
        except Exception as e:
            w = QWidget(); v = QVBoxLayout(w)
            err = QLabel(f"⚠ son_cockpit failed to load: {e}")
            err.setStyleSheet(f"color:{RED}; font-size:13px;")
            err.setWordWrap(True)
            v.addWidget(err); v.addStretch()
            return w
    ...
    def _build_advisors_tab(self):
        ...
        ctrl_box.setStyleSheet(f"QGroupBox {{ color: {GOLD}; }}")
        ...
        # Filter bar — decongested 2026-07-13. Was 6 widgets crammed inline
        # (wizard/strategist/autonomous/breed buttons truncating each other).
        # Now: filter combo + Refresh stay inline; the 4 action buttons live in
        # one "⚡ إجراءات ▾" dropdown (same pattern as the mission "⋯ MORE" menu).
        ...
        # ⚡ إجراءات — wizard / strategist / autonomous / breed, tucked away.
        actions_btn = QToolButton()
        actions_btn.setText("⚡ إجراءات ▾")
        ...
        actions_menu.setToolTipsVisible(True)   # QMenu hides QAction tooltips by default

        wiz_act = actions_menu.addAction("🎬 First-Run Wizard")
        wiz_act.setToolTip("Replay the visual onboarding stages")
        wiz_act.triggered.connect(self._show_onboarding_wizard)

        run_llm_act = actions_menu.addAction("🧠 Run LLM Strategist Now")
        run_llm_act.triggered.connect(
            lambda: self._advisor_run_now("llm_strategist"))

        # AUTONOMOUS toggle — checkable QAction. Refresh/handler code only
        # calls setText/setChecked/isChecked/blockSignals on this attribute,
        # all of which QAction supports, so the attribute stays intact.
        self._strat_autonomous_btn = QAction("⚙ AUTONOMOUS: OFF", actions_menu)
        ...
        self._strat_autonomous_btn.triggered.connect(self._toggle_strategist_autonomous)
        actions_menu.addAction(self._strat_autonomous_btn)

        actions_menu.addSeparator()
        breed_act = actions_menu.addAction("🧬 BREED top 2 BTC")
        breed_act.setToolTip("Cross top 2 HoF BTC genomes, backtest, admit child to HoF")
        breed_act.triggered.connect(lambda: self._breed_top_two("BTCUSDm"))

        actions_btn.setMenu(actions_menu)
        actions_btn.setStyleSheet(
            f"QToolButton {{ background: {BG_2}; color: {GOLD}; border: 1px solid {BORDER};"
            f"  border-radius: 6px; padding: 5px 12px; font-weight: 700; }}"
            f"QToolButton:hover {{ border-color: {GOLD}; }}"
            f"QToolButton::menu-indicator {{ image: none; }}")
        filter_bar.addWidget(actions_btn)

        filter_bar.addStretch()
        v.addLayout(filter_bar)

        # Live insight stream (read-only multi-line)
        stream_hdr = QLabel("📡 INSIGHT STREAM")
        stream_hdr.setStyleSheet(
            f"color: {GOLD}; font-size: 10px; font-weight: 700;"
            f" letter-spacing: 2px; padding-top: 4px;")
        v.addWidget(stream_hdr)
        self.advisor_stream = QPlainTextEdit()
        self.advisor_stream.setReadOnly(True)
        self.advisor_stream.setStyleSheet(
            f"background: {BG_1}; color: {TEXT}; "
            "font-family: 'Cascadia Mono', 'Consolas', monospace; "
            "font-size: 12px;")
        v.addWidget(self.advisor_stream, 1)

        # LLM Strategist last verdict (highlighted box)
        llm_hdr = QLabel("🧠 LLM STRATEGIST — LAST VERDICT")
        ...
        self.advisor_llm_box.setStyleSheet(
            f"background: {BG_2}; color: {TEXT}; "
            f"font-family: 'Cascadia Mono', 'Consolas', monospace; "
            f"border-left: 3px solid {GOLD}; padding: 6px;")
    ...
    def _refresh_advisors_panel(self) -> None:
        # NOTE: _u/_json were referenced below (autonomous sync + verdict box)
        # but never imported in this method — both blocks silently NameError'd
        # inside `except: pass` since day one. Fixed 2026-07-13.
        ...
        # Sync autonomous toggle state from server — via _bg_fetch so the HTTP
        # call never runs on the GUI thread inside this 3.5s refresh timer.
        def _fetch_autonomous():
            with _u.urlopen(
                "http://127.0.0.1:5055/api/r/agents/strategist_autonomous",
                timeout=2) as r:
                return _json.loads(r.read().decode())
        def _apply_autonomous(a):
        ...
        self._bg_fetch(_fetch_autonomous, _apply_autonomous)
        ...
        try:
            ...
            if sp.exists():
                ...
                txt = ""
                # DEGRADED banner: no LLM backend reachable. Show this loudly
                # so 'autonomous ON' is never displayed as healthy next to a
                # dead backend. The assessment below is last-known-good.
                if st.get("llm_degraded"):
                    txt += ("⛔ LLM DEGRADED — no backend reachable\n"
                            f"  last error: {st.get('last_error','?')}\n"
                            f"  last attempt: {st.get('last_attempt','?')}\n"
                            f"  tried: {st.get('tried','?')}\n"
                            "  (verdict below is STALE last-known-good)\n\n")
    ...
    def _build_hof_tab(self):
        ...
        # Palette-consistent card (was hardcoded #161826/#2a2d40 off-palette)
        self.hof_summary_lbl.setStyleSheet(
            f"font-size: 13px; color: {TEXT}; padding: 8px 10px; "
            f"background: {BG_1}; border: 1px solid {BORDER}; border-radius: 6px;")
        ...
        for i, g in enumerate(genomes):
            ...
            for col, txt in enumerate(cells):
                ...
                if col == 3:
                    ...
                    elif score >= 30: item.setForeground(QColor(GOLD))
    ...
    def _build_genes_tab(self):
        ...
        hb = QHBoxLayout()
        hb.setSpacing(8)
        # Monospace is required — _refresh_genes renders █░ pass-rate bars and
        # column-aligned rows that misalign in a proportional font.
        _genes_qss = ("background: {bg}; border: none; "
                      "font-family: 'JetBrains Mono','Consolas',monospace; "
        ...
        self.win_genes_list.setStyleSheet(_genes_qss.format(bg=BG_1, fg=GREEN))
        ...
        self.bad_genes_list.setStyleSheet(_genes_qss.format(bg=BG_1, fg=RED))
        bb.addWidget(self.bad_genes_list)
        hb.addWidget(bad_box)
        v.addLayout(hb)

        # Refresh row — auto-refreshes every 8s anyway (timer below), so the
        # manual button is compact + right-aligned, not a full-width slab.
        refresh_row = QHBoxLayout()
        auto_note = QLabel("auto-refreshes every 8s from Algory OOS learning")
        auto_note.setStyleSheet(f"color: {TEXT_MUTED}; font-size: 10px;")
        refresh_row.addWidget(auto_note)
    ...
    def _build_status_ticker(self):
        f = QFrame(); f.setMaximumHeight(34); f.setStyleSheet(f"background: {BG_1}; border-top: 1px solid {BORDER};")
        h = QHBoxLayout(f); h.setContentsMargins(12, 6, 12, 6)
    ...
    def _setup_terminal_dock(self):
        """Dock an interactive terminal panel to the right edge.

        Backed by a persistent PowerShell (QProcess) so cwd/env survive across
        commands. Toggleable via the sidebar '🖥 Terminal' button or Ctrl+`."""
        try:
            from r_native.terminal_widget import EmbeddedTerminal
            self.terminal = EmbeddedTerminal(cwd=str(PROJECT_ROOT), parent=self)
            ...
            self.terminal_dock.setAllowedAreas(
                Qt.RightDockWidgetArea | Qt.BottomDockWidgetArea | Qt.LeftDockWidgetArea)
            self.terminal_dock.setFeatures(
                QDockWidget.DockWidgetMovable | QDockWidget.DockWidgetFloatable
                | QDockWidget.DockWidgetClosable)
            self.terminal_dock.setStyleSheet(
                f"QDockWidget {{ color: {GOLD}; font-weight: 800; font-size: 11px;"
                f" titlebar-close-icon: none; }}"
                f"QDockWidget::title {{ background: {BG_1}; padding: 6px 10px;"
                f" border: 1px solid {BORDER}; border-radius: 6px;"
                f" letter-spacing: 1.5px; }}")
            self.addDockWidget(Qt.RightDockWidgetArea, self.terminal_dock)
            self.terminal_dock.resize(440, 600)
            # Start hidden — opens on demand to keep the boot view clean
            self.terminal_dock.hide()
            print("[ui] terminal dock ready", flush=True)
        except Exception as e:
            self.terminal = None
            self.terminal_dock = None
            print(f"[ui] terminal dock failed: {e}", flush=True)

    def _toggle_terminal(self):
        """Show/hide the embedded terminal dock (sidebar button + Ctrl+`)."""
        d = getattr(self, "terminal_dock", None)
        if not d:
            self._log("terminal unavailable")
            return
        if d.isVisible():
            d.hide()
        else:
    ...
    def graceful_shutdown(self):
        """Stop all owned QThreads + timers BEFORE QApplication teardown.

        Wired to app.aboutToQuit so it runs on every real exit path (tray
        Quit, heartbeat shutdown_hook, RSS watchdog). Without this the
        BackgroundPoller / Scan / Campaign QThreads get their C++ objects
        destroyed while run() is still looping -> Qt aborts with
        "QThread: Destroyed while thread is still running" -> the worker
        exits 0xFFFFFFFF and the launcher respawns it in a crash-loop.
        ...
        # Stop the Qt repaint tick so no new work is queued during teardown.
        try:
            if getattr(self, "tick", None):
                self.tick.stop()
        except Exception:
            pass

        # Background poller: cooperative stop (sets event) then join the QThread.
        try:
            poller = getattr(self, "poller", None)
            if poller is not None and poller.isRunning():
                poller.stop()              # signals the run() loop to exit
                if not poller.wait(3000):  # join up to 3s
                    poller.terminate()     # last resort — only if it won't stop
                    poller.wait(1000)
        except Exception:
            pass

        # Scan / campaign workers: only join if still running. On app exit we
        # must not leave a QThread C++ object dangling, so we wait briefly then
        # terminate if it refuses to finish.
        for attr in ("worker", "campaign_worker"):
            try:
        ...
        # Embedded shell — kill the child PowerShell so it doesn't orphan.
        try:
            if getattr(self, "terminal", None):
    ...
    def closeEvent(self, ev):
        # Cleanly stop the embedded shell so no orphan powershell lingers
        try:
            if getattr(self, "terminal", None):
                self.terminal.shutdown()
        except Exception:
    ...
    def _action_deploy(self):
        ...
        try:
            ...
            for col in range(self.vault_table.columnCount()):
                ...
                if it: it.setBackground(QColor(GOLD_BG))
    ...
    def _set_deploy_banner(self, text: str, error: bool = False, warning: bool = False):
        ...
        if error:
            bg, accent = RED_BG, RED       # red tint
        elif warning:
            bg, accent = GOLD_BG, GOLD     # amber tint
        else:
            bg, accent = GREEN_BG, GREEN   # green tint
        self._deploy_banner.setStyleSheet(
            f"background:{bg}; color:{TEXT}; padding:8px 12px; "
...
# Root-cause fix for duplicate-process storms: if R Native is launched
# twice (double double-click, stale launcher + new launcher, etc.) each
# app process spawns its OWN brain + r_executor + v2 stack. Two LIVE
# r_executors on the same magic double-submit orders and collide writing
# the same .tmp state files (the WinError 32 / WinError 5 floods). We bind
# a fixed loopback port as a process-wide mutex: only one app can hold it.
# A socket lock auto-releases on process death — no stale-lockfile risk.
_INSTANCE_LOCK_SOCKET = None  # keep a ref alive for the whole process lifetime


def _acquire_single_instance(port: int = 57321) -> bool:
    """Return True if we are the sole R Native instance, False if another
    one already holds the lock. Never raises."""
    global _INSTANCE_LOCK_SOCKET
    import socket as _socket
    try:
        s = _socket.socket(_socket.AF_INET, _socket.SOCK_STREAM)
        # No SO_REUSEADDR: we WANT the bind to fail if another instance holds it.
        s.bind(("127.0.0.1", port))
        s.listen(1)
        _INSTANCE_LOCK_SOCKET = s  # hold the ref so the OS keeps the port bound
        return True
    except OSError:
        return False
    except Exception:
        # Any unexpected failure → don't block startup (fail open).
        return True


def main():
    # SINGLE-INSTANCE GUARD — must run BEFORE any service spawns below.
    if not _acquire_single_instance():
        print("[unified] another R Native instance is already running — "
              "this duplicate will exit (single-instance guard).", flush=True)
        return
    ...
    # ── Graceful teardown on EVERY quit path ──────────────────────────────
    # tray "Quit", heartbeat shutdown_hook, and the RSS watchdog all call
    # app.quit(). aboutToQuit fires once, before Python tears down the C++
    # objects, so we join the BackgroundPoller / Scan / Campaign QThreads
    # here. Skipping this is what produced the recurring exit-0xFFFFFFFF
    # crash-loop ("QThread: Destroyed while thread is still running").
    app.aboutToQuit.connect(win.graceful_shutdown)
    ...
    try:
        ...
        # H.5: auto-restart at high RSS — belt-and-suspenders for any leaks
        # not caught by the post-campaign cleanup in _on_campaign_done.
        # Threshold resolves: env R_NATIVE_RSS_LIMIT_MB > settings.json
        # "rss_limit_mb" > 500 default. The monolith legitimately boots ~515MB
        # (PySide6 + WebEngine + 11 services), so data/r_native/settings.json
        # carries rss_limit_mb=1800 until the two-process split lands —
        # otherwise the 500 default would fire on every healthy startup.
        # Breach → wants_restart=true in /heartbeat (launcher respawns) →
        # 60s grace → graceful self-terminate (see heartbeat_server).
        heartbeat_server.start_rss_watchdog(
            threshold_mb = heartbeat_server.resolve_rss_limit_mb(),
            interval_s   = 30,
        )
    except Exception as e:
