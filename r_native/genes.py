...
class Genome:
    ...
    def crossover(cls, a: "Genome", b: "Genome", gen: int) -> "Genome":
        ...
        params = {}
        for p, (lo, hi) in CONT_PARAMS.items():
            chosen = a.params.get(p) if random.random() < 0.5 else b.params.get(p)
            if chosen is None:
                # Fall back to the other parent, then to the range midpoint,
                # so repair comparisons below never hit a None.
                chosen = a.params.get(p) if b.params.get(p) is None else b.params.get(p)
            if chosen is None:
                chosen = (lo + hi) // 2 if isinstance(lo, int) else round((lo + hi) / 2, 3)
            params[p] = chosen
        # Repair
        if not any(flags[e] for e in EXECUTION_GENES):
            flags[random.choice(EXECUTION_GENES)] = True
    ...
    def mutate(self, intensity: float = 0.10, gen: int = 0) -> "Genome":
        ...
        flags = Genome._fill_flag_defaults(self.flags)
        for g in ALL_GENES:
            if random.random() < intensity:
                flags[g] = not flags[g]
        params = Genome._fill_param_defaults(self.params)
        for p, (lo, hi) in CONT_PARAMS.items():
            if random.random() < intensity:
    ...
    @staticmethod
    def _fill_flag_defaults(flags: dict) -> dict:
        """Ensure every gene flag is present (default False).

        Malformed seeds may carry a partial flags dict; mutate() and the
        repair checks index flags by name and would KeyError on a missing
        gene. Backfill any absent gene as False.
        """
        flags = dict(flags or {})
        for g in ALL_GENES:
            if g not in flags:
    ...
    @staticmethod
    def _fill_param_defaults(params: dict) -> dict:
        """Ensure every continuous param is present and non-None.

        Seeds loaded from the Hall of Fame can be malformed (e.g. a flat
        summary dict with no nested ``params`` key), which leaves params
        empty or with None values. Crossover/mutate then compare
        ``None <= None`` and crash. Backfill any missing/None param from
        the midpoint of its CONT_PARAMS range so comparisons are always
        valid.
        """
        params = dict(params or {})
        for p, (lo, hi) in CONT_PARAMS.items():
            if params.get(p) is None:
                params[p] = (lo + hi) // 2 if isinstance(lo, int) else round((lo + hi) / 2, 3)
        return params

    @classmethod
    def from_dict(cls, d: dict) -> "Genome":
        return cls(flags=cls._fill_flag_defaults(d.get("flags", {})),
                   params=cls._fill_param_defaults(d.get("params", {})),
