...
# The 24 KPIs above, organised into labelled sections so the STATS panel reads
# as grouped cards instead of one long crammed 2-column list (user: "محشور حشر").
# set_stats() is unaffected — it still looks cells up by key, order is cosmetic.
STATS_GROUPS: list[tuple[str, list[str]]] = [
    ("الأداء · PERFORMANCE",  ["net_profit", "drawdown", "profit_factor",
                                "sharpe", "recovery_factor"]),
    ("الصفقات · TRADES",       ["total_trades", "win_rate", "long_trades",
                                "short_trades", "avg_hold_time"]),
    ("داخل/خارج العيّنة · IS / OOS", ["is_return", "is_trades", "oos_return",
                                "oos_trades", "linearity", "persistence"]),
    ("الربح والخسارة · P&L",   ["avg_profit", "avg_loss", "biggest_win",
                                "biggest_loss"]),
    ("السلاسل · STREAKS",      ["max_win_streak", "max_loss_streak",
...
def _pill_style(bg: str) -> str:
    """Rounded pass/fail chip used by the SCORE tab."""
    return (f"background: {bg}; color: {BG_0}; font-size: 10px; font-weight: 800;"
...
class StatsTab(QWidget):
    def __init__(self, parent: QWidget | None = None) -> None:
        ...
        _meta = {k: lbl for lbl, k, _knd in STATS_KPIS}
        r = 0
        for gtitle, keys in STATS_GROUPS:
            hdr = QLabel(gtitle)
            hdr.setStyleSheet(
                f"color: {GOLD}; font-size: 10px; font-weight: 800; "
                f"letter-spacing: 1px; padding: 8px 2px 2px 2px;")
            grid.addWidget(hdr, r, 0, 1, 2)
            r += 1
            for j, key in enumerate(keys):
                cell = _KpiCell(_meta.get(key, key))
                grid.addWidget(cell, r + j // 2, j % 2)
...
class ScoreTab(QWidget):
    def __init__(self, parent: QWidget | None = None) -> None:
        ...
        outer.setSpacing(6)   # 4-6px breathing room between requirement rows
        title = QLabel("PURGE REQUIREMENTS")
        title.setStyleSheet(
            f"color: {GOLD}; font-size: 10px; font-weight: 800; letter-spacing: 2px;"
            " padding: 2px 0 4px 0;")
        outer.addWidget(title)
        # Fixed stretch factors — header and every row share 3/2/2/1 so the
        # three data columns stay perfectly aligned.
        _STRETCH = [("CRITERION", 3), ("REQUIRED", 2), ("ACTUAL", 2), ("STATUS", 1)]
        header = QFrame()
        header.setStyleSheet(f"background: {BG_3}; border: 1px solid {BORDER};")
        hl = QHBoxLayout(header)
        hl.setContentsMargins(8, 4, 8, 4)
        for text, stretch in _STRETCH:
            lab = QLabel(text)
            lab.setStyleSheet(
        ...
        for label, req_key, _stat_key in SCORE_ROWS:
            ...
            pill.setStyleSheet(_pill_style(MUTED))
            # Center the chip inside its column so it renders as a compact
            # rounded badge instead of a full-width bar.
            rl.addWidget(pill, 1, Qt.AlignmentFlag.AlignCenter)
            outer.addWidget(row)
    ...
    def set_score(self, purge_req: dict[str, Any], stats: dict[str, Any]) -> None:
        for label, req_key, stat_key in SCORE_ROWS:
            ...
            if req_val is None or act_val is None:
                r["pill"].setText("—")
                r["pill"].setStyleSheet(_pill_style(MUTED))
            elif passed:
                r["pill"].setText("PASS")
                r["pill"].setStyleSheet(_pill_style(GREEN))
            else:
                r["pill"].setText("FAIL")
                r["pill"].setStyleSheet(_pill_style(RED))
...
class _ClassRow(QFrame):
    ...
    def __init__(self, label: str, key: str, tip: str, parent: QWidget | None = None) -> None:
        ...
        lay.setSpacing(8)
        # Definition-list look: muted letter-spaced key, wrapped bright value.
        name = QLabel(label)
        name.setStyleSheet(
            f"color: {MUTED}; font-size: 10px; font-weight: 800; letter-spacing: 1.5px;"
        )
...
class TradesTab(QWidget):
    def __init__(self, parent: QWidget | None = None) -> None:
        ...
        self.table.setStyleSheet(
            ...
            f" color: {TEXT}; gridline-color: {BORDER}; border: 1px solid {BORDER};"
            " font-family: Consolas; font-size: 11px; }"
            f"QTableWidget::item {{ padding: 2px 6px; }}"
            f"QHeaderView::section {{ background: {BG_3}; color: {MUTED};"
            f" border: 1px solid {BORDER}; padding: 4px 6px; font-weight: 800;"
...
class GenomeTab(QWidget):
    ...
    def __init__(self, parent: QWidget | None = None) -> None:
        ...
        self.params_grid.setVerticalSpacing(3)
        ...
        self.flags_grid = QGridLayout(); self.flags_grid.setSpacing(4)
    ...
    def _section_title(self, txt: str) -> QLabel:
        ...
        lbl.setStyleSheet(
            ...
            " padding: 8px 0 4px 0;")   # 8px top margin separates sections
        return lbl
    ...
    def set_genome(self, params: dict, active_genes: list, flags: dict, exit_breakdown: dict):
        ...
        # Gene flags — compact rounded chips, 3 per row (GREEN = active, dim = off)
        ...
        if not flags:
            ...
            for i, (k, v) in enumerate(items):
                ...
                chip = QLabel(k.replace("use_", ""))
                if v:
                    chip.setStyleSheet(
                        f"color: {GREEN}; border: 1px solid {GREEN};"
                        f" background: {BG_3}; border-radius: 8px; padding: 1px 6px;"
                ...
                # Left-align inside the cell so the chip hugs its text instead
                # of stretching to full column width.
                self.flags_grid.addWidget(
                    chip, row, col,
...
class EquityCurveWidget(QWidget):
    ...
    def paintEvent(self, _ev):
        ...
        # ── IS/OOS split marker ──
        _legend_font = QFont("Consolas")
        _legend_font.setPixelSize(11)
        _legend_font.setBold(True)
        if 0 < split < n:
            x_split = int(X(split))
            ...
            p.drawText(x_split + 3, pad_t + 13, "OOS")

        # ── Legend: IS (white) / OOS (cyan) — top-right, 11px ──
        p.setFont(_legend_font)
        lx = W - pad_r - 96
        ly = pad_t + 8
        p.setPen(QPen(QColor("#f1f5f9"), 2))
        p.drawLine(lx, ly, lx + 14, ly)
        p.setPen(QColor("#f1f5f9"))
        p.drawText(lx + 18, ly + 4, "IS")
        p.setPen(QPen(QColor("#22d3ee"), 2))
        p.drawLine(lx + 40, ly, lx + 54, ly)
        p.setPen(QColor("#22d3ee"))
        p.drawText(lx + 58, ly + 4, "OOS")
        ...
        # ── Footer: net (colored) + drawdown/trades sub-label (MUTED) ──
        _footer_font = QFont("Consolas")
        _footer_font.setPixelSize(11)
        p.setFont(_footer_font)
        net = eqs[-1] - eqs[0]
        max_dd = abs(min(dds)) if dds else 0
        net_color = GREEN if net >= 0 else RED
        net_str = f"Net: {'+' if net >= 0 else ''}${net:.2f}"
        p.setPen(QColor(net_color))
        p.drawText(pad_l, H - 8, net_str)
        p.setPen(QColor(MUTED))
        p.drawText(pad_l + p.fontMetrics().horizontalAdvance(net_str), H - 8,
                   f"   ·   Max DD: ${max_dd:.2f}   ·   Trades: {n - 1}")
...
class InspectorPanel(QFrame):
    ...
    def __init__(self, parent: QWidget | None = None) -> None:
        ...
        self.setStyleSheet(
            ...
            f"QTabBar::tab {{ background: {BG_2}; color: {MUTED}; padding: 6px 7px;"
            " font-weight: 700; letter-spacing: 0px; font-size: 10px;"
        ...
        # Compact, non-overflowing in the narrow inspector — no ◄► scroll arrows.
        self.top_tabs.setUsesScrollButtons(False)
        self.top_tabs.tabBar().setExpanding(False)
        self.top_tabs.setElideMode(Qt.ElideNone)
