...
SERVICES = [
    ...
    # NOTE (2026-05-30): the v2 ollama_council was REMOVED here — it duplicated
    # the user's pre-existing 7-role root council (C:\Users\Radhi\MT5\
    # ollama_council.py, watchdog-managed) and the two fought over the single
    # GPU (one model resident at a time → swap-thrash → timeouts). Root council
    # keeps the GPU. The v2 module (runtime/ollama_council.py) stays on disk,
    # dormant, until its governance-hook + per-symbol honesty-ledger are merged
    # INTO the root council (the real consolidation) — pending user's nod.
]
