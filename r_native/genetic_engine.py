...
def _evaluate_genome_worker(payload: tuple) -> dict:
    ...
    try:
        ...
        # H.4: per-worker-process LRU bar cache — every genome eval in this
        # pool worker reuses the same (symbol, tf, bars_back) array instead of
        # re-pulling it from the terminal thousands of times per campaign.
        # max_age_s=600 keeps one consistent snapshot per campaign phase.
        try:
            from r_native.bar_cache import fetch as _bars_fetch
            bars = _bars_fetch(symbol, tf_name, bars_back,
                               lambda: mt5.copy_rates_from_pos(symbol, tf, 0, bars_back),
                               max_age_s=600.0)
        except ImportError:
