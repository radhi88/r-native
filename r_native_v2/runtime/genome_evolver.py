...
# Sensible fallbacks for genomes bred elsewhere (e.g. the seeded champion)
# that don't carry the evolver's full param set — keeps crossover crash-proof.
_PARAM_DEFAULTS = {
...
def crossover(parent_a: dict, parent_b: dict) -> dict:
    ...
    Key-safe: a parent missing a gene falls back to the other parent, then to a
    sane default. Foreign-bred genomes (champion, FVG child) lack some keys, and
    a bare KeyError used to abort the whole evolution cycle."""
    child = {}
    ...
    for k in keys:
        ...
        other = parent_b if src is parent_a else parent_a
        child[k] = src.get(k, other.get(k, _PARAM_DEFAULTS.get(k)))
...
def mutate(genome: dict, intensity: float = 0.15) -> dict:
    ...
    for param, (low, high) in PARAM_RANGES.items():
        if random.random() < intensity:
            cur = new.get(param, _PARAM_DEFAULTS.get(param, low))
            if isinstance(low, int):
                new[param] = max(low, min(high, cur + random.choice([-1, 1])))
            else:
                delta = (high - low) * 0.1 * random.choice([-1, 1])
                new[param] = round(max(low, min(high, cur + delta)), 2)
    new["mutated_from"] = genome.get("name", "unknown")
...
def main_loop():
    ...
    while True:
        try:
            ...
            # Operator duty: vote on peers' open governance proposals so safe
            # changes can reach consensus (the user wants agents to act, and
            # judging peers' work is half of being an operator).
            try:
                from runtime.shared.agent_operator import AgentOperator
                votes = AgentOperator("genome_evolver").review_peers()
                if votes:
                    print(f"  peer-review: voted on {len(votes)} proposals → {votes}")
            except Exception as _e:
                print(f"  peer-review skipped: {_e}")
