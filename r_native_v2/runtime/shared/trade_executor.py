...
def execute_signal(
    ...
    if sig.order_type == "MARKET":
        ...
        # ── pending order (LIMIT / STOP) anchored to sig.entry ──────
        entry_price = float(sig.entry)
        ref = tick.ask if sig.side == "BUY" else tick.bid
        # Resolve LIMIT vs STOP against the live tick so callers can stay
        # declarative: a BUY below price = LIMIT, a BUY above price = STOP.
        if sig.side == "BUY":
            order_type = (mt5.ORDER_TYPE_BUY_LIMIT if entry_price < ref
                          else mt5.ORDER_TYPE_BUY_STOP)
        else:
            order_type = (mt5.ORDER_TYPE_SELL_LIMIT if entry_price > ref
        ...
        req = {
            ...
            "deviation": deviation,
            "magic": int(sig.magic),
            "comment": (comment or f"{sig.source[:8]}_{sig.side}P")[:31],
            "type_filling": mt5.ORDER_FILLING_RETURN,
        }
        if sig.expire_s and sig.expire_s > 0:
            from datetime import timedelta
            req["type_time"] = mt5.ORDER_TIME_SPECIFIED
            req["expiration"] = int(
                (datetime.now(timezone.utc) + timedelta(seconds=int(sig.expire_s))).timestamp())
        else:
            req["type_time"] = mt5.ORDER_TIME_GTC

    r = mt5.order_send(req)
    if (not r or r.retcode != mt5.TRADE_RETCODE_DONE) and sig.order_type == "MARKET":
