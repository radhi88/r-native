...
class TradeSignal:
    ...
    order_type: OrderType = "MARKET"     # MARKET=instant fill; LIMIT/STOP=pending
    entry: Optional[float] = None        # required for LIMIT/STOP — the pending price
    expire_s: int = 0                    # seconds until pending order auto-expires (0=GTC)
    ...
    def validate(self) -> None:
        ...
        if self.order_type not in ("MARKET", "LIMIT", "STOP"):
            raise ContractError(f"{self.source}: bad order_type {self.order_type!r}")
        if self.order_type in ("LIMIT", "STOP"):
            if self.entry is None or self.entry <= 0:
                raise ContractError(
                    f"{self.source}: {self.order_type} requires a positive entry price")
            # SL/TP must straddle the pending entry, not the live tick
            if self.side == "BUY" and not (self.sl < self.entry < self.tp):
                raise ContractError(
                    f"{self.source}: BUY pending requires sl < entry < tp")
            if self.side == "SELL" and not (self.tp < self.entry < self.sl):
                raise ContractError(
                    f"{self.source}: SELL pending requires tp < entry < sl")
            # LIMIT vs STOP semantics are resolved at execution time against the
            # live tick (BUY_LIMIT below ask, BUY_STOP above ask, etc.).
