...
class DB:
    ...
    def _conn(self) -> sqlite3.Connection:
        ...
        if c is None:
            # WAL pragma retry (2026-05-30, QA audit): a fleet-wide relaunch has
            # ~15 processes hitting this simultaneously on a 140MB db; the WAL
            # handshake can throw a transient 'disk I/O error' that used to KILL
            # services at boot (champion_evolution + outcome_filler, 03:11).
            # 3 attempts with backoff turns that race into a 3-second wait.
            ...
            if c is None:
                raise last_err or sqlite3.OperationalError("db connect failed")
            self._local.conn = c
        return c
