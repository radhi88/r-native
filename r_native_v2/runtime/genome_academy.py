...
CYCLE_SECONDS = 60
# F2-a.5: the GA fitness basket now comes from the single source of truth
# (symbol_universe, gauntlet flag). This is NOT the trading set — genomes are scored
# on how well they GENERALIZE across this curated benchmark (metals+FX majors/cross+
# crypto). Identity parallel-run 2026-06-07: gauntlet_symbols() == legacy 7 (zero GA
# fitness change). F2-c may expand this basket. Fail-safe: fall back to the legacy 7
# if the universe import/read fails, so the academy never trains on a broken set.
_LEGACY_SYMBOLS = ["XAUUSDm", "EURUSDm", "GBPUSDm", "GBPJPYm", "BTCUSDm", "USDJPYm", "XAGUSDm"]
try:
    from runtime.shared.symbol_universe import gauntlet_symbols as _gauntlet_symbols
    SYMBOLS = _gauntlet_symbols() or _LEGACY_SYMBOLS
except Exception:  # noqa: BLE001 — academy must never crash on universe import
