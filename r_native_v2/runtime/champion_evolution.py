...
def evolve_once() -> dict:
    ...
    if chal_fit > champ_fit * CHALLENGE_MARGIN and chal_fit > 0:
        # The son found a better brain — but DEPLOYING it is the user's
        # "قبل اطلاقها يستشيرونك" moment. We no longer crown unilaterally; we
        # FILE A PROPOSAL through governance. crown_genome is always HIGH, so it
        # lands in Claude's consult queue; the governor performs the actual swap
        # (live_genome + champion file + unified_trader restart) once approved.
        try:
            from runtime.shared.agent_operator import AgentOperator
            op = AgentOperator("champion_evolution")
            pid = op.crown(
                challenger, fitness=chal_fit,
                dethroned=champion.get("name") or "",
                name=challenger.get("name"),
                rationale=(f"challenger {challenger.get('name')} fitness {chal_fit} "
                           f"beats champion {champion.get('name')} ({champ_fit}) "
                           f"by ≥{CHALLENGE_MARGIN}x — request crown"))
            result["action"] = "PROPOSED_CROWN"
            result["proposal_id"] = pid
            with LINEAGE.open("a", encoding="utf-8") as f:
                f.write(json.dumps(result, default=str) + "\n")
        except Exception as e:
            # Governance unavailable → do NOT silently auto-crown money path;
            # hold the champion and surface the error for the next cycle.
            result["action"] = "propose_failed"
...
def main():
    ...
    while True:
        try:
            ...
            if r["action"] == "PROPOSED_CROWN":
                print(f"[{ts}] 📨 PROPOSED CROWN {r['challenger']} "
                       f"(fit {r['chal_fitness']} > champ {r['champ_fitness']}) — "
                       f"awaiting Claude consult [proposal {r.get('proposal_id')}]")
            elif r["action"] == "propose_failed":
                print(f"[{ts}] ⚠ crown proposal failed: {r.get('error')} — champion holds")
