...
# BTCUSDm added 2026-05-30: crypto trades 24/7, so the son keeps working on the
# weekend when FX/metals markets are closed ("خله يشتغل على البيتكوين دام باقي
# العملات مغلق سوقها"). Its snapshot feeds brain_live__BTCUSDm.json like the rest.
# F2-a.4: observation set now comes from the single source of truth (symbol_universe).
# brain_v1 is an OBSERVER (writes per-symbol snapshots, never trades), so it watches the
# FULL universe (enabled or not) via all_symbols() — the `enabled` flag gates TRADING, not
# observation. Fail-safe: if the universe import/read fails, fall back to the legacy list so
# the son never goes blind. (Parallel-run 2026-06-06: universe == legacy 11 + GBPJPYm.)
_LEGACY_SYMBOLS = ["XAUUSDm", "EURUSDm", "GBPUSDm", "USDJPYm",
                   "USDCADm", "AUDUSDm", "NZDUSDm", "USDCHFm", "EURJPYm", "XAGUSDm",
                   "BTCUSDm"]
try:
    import sys as _sys_u
    from pathlib import Path as _P_u
    _sys_u.path.insert(0, str(_P_u(__file__).resolve().parent.parent))
    from runtime.shared.symbol_universe import all_symbols as _all_symbols
    SYMBOLS = _all_symbols() or _LEGACY_SYMBOLS
except Exception:  # noqa: BLE001 — observer must never crash on universe import
...
# BTCUSDm: point 0.01 → 2 decimals (live MT5 confirms digits=2, not 1).
_DIGITS = {"XAUUSDm": 2, "XAGUSDm": 3, "BTCUSDm": 2, "EURUSDm": 5,
...
def _find_fvgs(bars, lookback=30, digits=2):
    # digits MUST match the symbol's price precision — rounding a 5-digit FX
    # gap (0.72145→0.72158) to 2 dp collapses it to 0.72→0.72 size 0.0 and the
    # downstream FVG-pending planner discards it as a degenerate gap. Always
    # pass the per-symbol digits from _DIGITS so FX gaps survive.
    bull, bear = [], []
    n = min(lookback, len(bars) - 2)
    for i in range(len(bars) - 2, len(bars) - n - 2, -1):
        if i < 1: continue
        ...
        if c["low"] > a["high"]:
            ...
            if min_after > bot:
                bull.append({"bot": round(bot,digits), "top": round(top,digits), "mid": round((top+bot)/2,digits),
                              "size": round(top-bot,digits), "age": len(bars)-1-(i+1)})
        if c["high"] < a["low"]:
            top, bot = a["low"], c["high"]
            max_after = max((b["high"] for b in bars[i+2:]), default=bot)
            if max_after < top:
                bear.append({"bot": round(bot,digits), "top": round(top,digits), "mid": round((top+bot)/2,digits),
                              "size": round(top-bot,digits), "age": len(bars)-1-(i+1)})
...
def capture(mt5, sym=SYMBOL) -> dict:
    ...
    snap = {
        ...
        "fvg_m5":  _find_fvgs(m5, digits=dg),
        "fvg_m15": _find_fvgs(m15, digits=dg),
        "zigzag_m5": pivots,
    ...
    try:
        ...
        snap["footprint"] = _ff(snap.get("bid", 0), _atr_m5, symbol=snap.get("symbol"))
    except Exception:
        snap["footprint"] = {}
